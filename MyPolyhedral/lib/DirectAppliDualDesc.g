...
ComputeIntersectionOfCones:=function(ListEXT)
  local ListEquaTot, BasisIntersection, GetInequality, ListIneqA, ListIneqA_Red, ListIneqB, EXTred, TheBasis, TheSpa, ePointInt, ePointIntB, EXT, eSol, dimSpa, eVectZero, pos;
  ListEquaTot:=Concatenation(List(ListEXT, x->NullspaceMat(TransposedMat(x))));
  BasisIntersection:=NullspaceMat(TransposedMat(ListEquaTot));
  dimSpa:=Length(BasisIntersection);
  eVectZero:=ListWithIdenticalEntries(dimSpa, 0);
  ...
  ListIneqA:=Concatenation(List(ListEXT, GetInequality));
  ListIneqA_Red:=Set(List(ListIneqA, RemoveFraction));
  TheSpa:=LinearDeterminedByInequalities(ListIneqA_Red);
  TheBasis:=TheSpa * BasisIntersection;
  ListIneqB:=List(ListIneqA_Red, x->List(TheSpa, y->y*x));
  EXTred:=DualDescription(ListIneqB);
  return List(EXTred * TheBasis, RemoveFraction);
end;
