...
# Computing the NullspaceMat of a matrix.
SAGE_NullspaceMat:=function(eMat)
  local FileI, FileO, FileSTDOUT, FileSTDERR, output, IsFirstLine, IsFirstCol, eLine, eVal, nbRow, nbCol, TheRes, TheCommand, Python3;
  FileI:=Filename(POLYHEDRAL_tmpdir, "Nullspace_sage.input");
  FileO:=Filename(POLYHEDRAL_tmpdir, "Nullspace_sage.output");
  FileSTDOUT:=Filename(POLYHEDRAL_tmpdir, "Nullspace_sage.stdout");
  FileSTDERR:=Filename(POLYHEDRAL_tmpdir, "Nullspace_sage.stderr");
  output:=OutputTextFile(FileI, true);
  nbRow:=Length(eMat);
  nbCol:=Length(eMat[1]);
  AppendTo(output, "A = matrix(QQ, [");
  IsFirstLine:=true;
  for eLine in eMat
  do
  ...
  od;
  AppendTo(output, "])\n");
  AppendTo(output, "eKer = A.left_kernel()\n");
  AppendTo(output, "TheBasis = eKer.basis()\n");
  AppendTo(output, "dim=len(TheBasis)\n");
  AppendTo(output, "strO=\"return [\"\n");
  AppendTo(output, "for i in range(dim):\n");
  AppendTo(output, "    if i > 0:\n");
  AppendTo(output, "        strO += \",\\n\"\n");
  AppendTo(output, "    strO += \"[\"\n");
  AppendTo(output, "    eVect = TheBasis[i]\n");
  AppendTo(output, "    for j in range(", nbRow, "):\n");
  AppendTo(output, "        if j > 0:\n");
  AppendTo(output, "            strO += \",\"\n");
  AppendTo(output, "        strO += str(eVect[j])\n");
  AppendTo(output, "    strO += \"]\"\n");
  AppendTo(output, "strO += \"];\"\n");
  AppendTo(output, "f = open(\"", FileO, "\", \"w\")\n");
  AppendTo(output, "f.write(strO)\n");
  AppendTo(output, "f.close()\n");
  CloseStream(output);
  #
  TheCommand:=Concatenation("sage < ", FileI, " > ", FileSTDOUT, " 2> ", FileSTDERR);
  Exec(TheCommand);
  TheRes:=ReadAsFunction(FileO)();
  RemoveFileIfExist(FileO);
  RemoveFileIfExist(FileI);
  RemoveFileIfExist(FileSTDOUT);
  RemoveFileIfExist(FileSTDERR);
...
AntipodalCanonicalization:=function(eVect)
    local len, i;
    len:=Length(eVect);
    for i in [1..len]
    do
...
WriteListMatrixFile:=function(eFile, LMat)
    local output;
    output:=OutputTextFile(eFile, true);
    WriteListMatrix(output, LMat);
    CloseStream(output);
end;
...
GetUniquenessIndices:=function(ListEnt)
    local eSet;
    eSet:=Set(ListEnt);
    return List(eSet, x->Position(ListEnt,x));
end;
...
ProjectionLinearSpace:=function(QuadForm, BasisSpace)
    local n, QuadFormProj, OrthSpace, TotSpace, TheProj, i, ProjectionMatrix, IntegralAntecedent, InProjection;
    n:=Length(QuadForm);
    QuadFormProj:=BasisSpace * QuadForm * TransposedMat(BasisSpace);
    OrthSpace:=NullspaceMat(QuadForm * TransposedMat(BasisSpace));
    TotSpace:=Concatenation(BasisSpace, OrthSpace);
    TheProj:=NullMat(n, n);
    for i in [1..Length(BasisSpace)]
    do
        TheProj[i][i]:=1;
    od;
    ProjectionMatrix:=Inverse(TotSpace) * TheProj * TotSpace;
    IntegralAntecedent:=function(eVect)
        local eVectP, TheConcat, eSol, TheMult;
        eVectP:=eVect * BasisSpace;
        TheConcat:=Concatenation(ProjectionMatrix, [eVectP]);
        TheMult:=RemoveFractionMatrixPlusCoef(TheConcat).TheMult;
        eSol:=SolutionIntMat(TheMult * ProjectionMatrix, TheMult * eVectP);
        if eSol=fail then
            Error("The vector has no integral solution");
        fi;
        return eSol;
    end;
    InProjection:=function(eVect)
        local eVectP;
        eVectP:=eVect * ProjectionMatrix;
        return SolutionMat(BasisSpace, eVectP);
    end;
    return rec(QuadFormProj:=QuadFormProj, ProjectionMatrix:=ProjectionMatrix,
               IntegralAntecedent:=IntegralAntecedent, InProjection:=InProjection);
end;
...
__FindFacetInequalityGeneral:=function(EXT, ListIncidence, IsPos, RemoveFrac)
  ...
  if Length(NLE)<>1 then
    return rec(success:=false, reason:=Concatenation("|NLE|=", String(Length(NLE)), " instead of expected 1\n"));
  fi;
  NSP:=NLE[1];
...
__FindFacetInequality:=function(EXT, ListIncidence)
  local IsPos, RemoveFrac, Nval, TheReply;
  if IsMatrixRational(EXT)=true then
    IsPos:=function(x)
      return x>0;
    end;
    RemoveFrac:=RemoveFraction;
    TheReply:=__FindFacetInequalityGeneral(EXT, ListIncidence, IsPos, RemoveFrac);
#    Print("TheReply=", TheReply, "\n");
    return TheReply.eIneq;
  fi;
  for Nval in [2,5]
  do
    if QN_IsMatrix(Nval, EXT)=true then
...
SYMPOL_PrintVectorStream:=function(output, eV)
    local eEnt;
    AppendTo(output, Length(eV), "\n");
    for eEnt in eV
    do
...
SYMPOL_PrintMatrixStream:=function(output, EXT)
...
SYMPOL_PrintMatrix:=function(eFile, EXT)
  ...
  output:=OutputTextFile(eFile, true);
  SYMPOL_PrintMatrixStream(output, EXT);
  CloseStream(output);
end;
...
SYMPOL_ReadGroup:=function(eFile)
    local ListLines, n, nGen, ListGen, iGen, eGen, GRP;
    ...
    for iGen in [1..nGen]
    do
        eGen:=PermList(List(ListLines[iGen+1], x->x+1));
        Add(ListGen, eGen);
    od;
    GRP:=PersoGroupPerm(ListGen);
    return [n, GRP];
end;
...
# We want to find the x in R^p such that
# Avect + x Bmat is integral
IntegralLinearSpace:=function(Avect, Bmat)
    local Bmat_tr, Bmat_Zbasis_tr, Ymat_tr, Zmat_tr, Zmat, Ymat, Bmat_Zbasis, DiffA, Avect_Zbasis, eRec, Bmat_red, x0_red, x0, BasisInt, BasisInt_red, f_extend;
    # First we reduce the number of columns of Bmat
    Bmat_tr:=TransposedMat(Bmat);
    Bmat_Zbasis_tr:=GetZbasis(Bmat_tr);
    # Bmat_tr = Zmat_tr Bmat_Zbasis_tr
    # Bmat_Zbasis_tr = Ymat_tr Bmat_tr
    Ymat_tr:=List(Bmat_Zbasis_tr, x->SolutionIntMat(Bmat_tr, x));
    Zmat_tr:=List(Bmat_tr, x->SolutionIntMat(Bmat_Zbasis_tr, x));
    # Bmat = Bmat_Zbasis Zmat
    # Bmat_Zbasis = Bmat Ymat
    Zmat := TransposedMat(Zmat_tr);
    Ymat := TransposedMat(Ymat_tr);
    Bmat_Zbasis:=TransposedMat(Bmat_Zbasis_tr);
    # So, we multiply by Zmat
    DiffA:=Avect - Avect * Ymat * Zmat;
    if IsIntegralVector(DiffA)=false then
        return fail;
    fi;
    # We multiply and this gets us
    # Avect_Zbasis + x Bmat_Zbasis
    # With Bmat_Zbasis of full column rank.
    Avect_Zbasis:=Avect * Ymat;
    # Since the x coefficients are real,
    # we select the rows to get a full rank matrix Bmat_red
    # The equation becomes
    # Avect_Zbasis + x Bmat_red
    eRec:=RowReduction(Bmat_Zbasis);
    Bmat_red:=eRec.EXT;
    if Length(Bmat_red)<>Length(Bmat_red[1]) then
        Error("Bmat_red should be a square matrix");
    fi;
    if RankMat(Bmat_red)<>Length(Bmat_red) then
        Error("Bmat_red should be an invertible matrix");
    fi;
    x0_red:=- Avect_Zbasis * Inverse(Bmat_red);
    f_extend:=function(u_red)
        local u;
        u:=ListWithIdenticalEntries(Length(Bmat), 0);
        u{eRec.Select}:=u_red;
        return u;
    end;
    x0:=f_extend(x0_red);
    BasisInt_red:=Inverse(Bmat_red);
    BasisInt:=List(BasisInt_red, f_extend);
