...
LinPolytope_Invariant:=function(EXT)
    local UseCPP, TheLimit;
    UseCPP:=true;
    if UseCPP then
        return LinPolytope_Invariant_CPP(EXT);
    else
        TheLimit:=500;
...
LinPolytope_Automorphism_CPP:=function(EXT)
    local FileEXT, FileOUT, FileERR, GRP;
    FileEXT:=Filename(POLYHEDRAL_tmpdir, "Input.ext");
    FileOUT:=Filename(POLYHEDRAL_tmpdir, "Input.out");
    FileERR:=Filename(POLYHEDRAL_tmpdir, "Input.err");
    WriteMatrixFile(FileEXT, EXT);
    #
    Exec(FileGRP_LinPolytope_Automorphism, " rational ", FileEXT, " GAP ", FileOUT, " 2> ", FileERR);
...
LinPolytope_Automorphism:=function(EXT)
    local UseCPP, EXTred, n, Qmat, eEXT, Qinv;
    UseCPP:=true;
    if UseCPP then
        return LinPolytope_Automorphism_CPP(EXT);
    else
...
GetScalarMatrixInvariant_PolytopeStabSubset_AddMat:=function(EXT, EXTsub, ListAddMat)
    ...
    if UseMethod="CPP" then
        ...
        CloseStream(output);
        #
        Exec(FileGRP_Invariant_ListMat_Diag_EXT, " ", FileI, " ", FileO);
...
LinPolytope_AutomorphismStabSubset_AddMat:=function(EXT, EXTsub, ListAddMat)
    ...
    if UseMethod="CPP" then
        ...
        CloseStream(output);
        #
        Exec(FileGRP_ComputeAut_ListMat_Diag_EXT, " ", FileI, " GAP ", FileO);
...
LinPolytope_IsomorphismStabSubset_AddMat:=function(EXT1, EXTsub1, EXT2, EXTsub2, ListAddMat1, ListAddMat2)
    ...
    if UseMethod="CPP" then
        ...
        CloseStream(output);
        Exec(FileGRP_TestEquivalence_ListMat_Diag_EXT, " ", FileI, " GAP ", FileO);
