...
# uses only the center of the Delaunay
Lattice_Stabilizer_Method2:=function(DataLattice, ThePoint)
...
# We iterate over the denominators of the center of the Delaunay polytope.
Lattice_Stabilizer_Method3:=function(DataLattice, ThePoint)
...
# We iterate over all the elements of the isometry group
Lattice_Stabilizer_Method4:=function(DataLattice, EXT, GRP)
...
# This method uses the subspace method.
# That is we start from the rational group and find
# the integral part.
Lattice_Stabilizer_Method7:=function(DataLattice, EXT, GRP)
...
# We extend here by an invariant basis (which has to be of full rank
# but does not need to Z-span Z^n). 
Lattice_Stabilizer_Method8:=function(DataLattice, EXT)
...
# We extend here by an invariant basis (which has to be a Z basis of Z^n)
Lattice_Stabilizer_Method9:=function(DataLattice, EXT)
  local RecMat, GRPbig, ListMatGens, eGen, eBigMat, eMat, GRPret, ListBigMat;
  RecMat:=GetScalMat_Method8(DataLattice, EXT);
  GRPbig:=LinPolytope_Automorphism_GramMat(RecMat.EXText, RecMat.Qmat);
  Print("|GRPbig| = ", Order(GRPbig), "\n");
  ListMatGens:=[];
  ListBigMat:=[];
  for eGen in GeneratorsOfGroup(GRPbig)
  do
      eBigMat:=FindTransformation(RecMat.EXText, RecMat.EXText, eGen);
      Add(ListBigMat, eBigMat);
      eMat:=FuncExplodeBigMatrix(eBigMat).MatrixTransformation;
      if IsIntegralMat(eMat)=false then
          Error("The metrix should be integral");
      fi;
      Add(ListMatGens, eMat);
  od;
  GRPret:=PersoGroupMatrix(ListMatGens, DataLattice.n);
  SetSize(GRPret, Order(GRPbig));
  return GRPret;
end;


GetScalMat_Method9:=function(DataLattice, EXT, ThePoint)
  local SVR_EXT, EXText, CP, TheCenter, CharPair, nbV, ScalMat, i, j, eScal;
  SVR_EXT:=List(DataLattice.InvariantBase, x->Concatenation([0],x));
  EXText:=Concatenation(EXT, SVR_EXT, [ThePoint]);
  CP:=CenterRadiusDelaunayPolytopeGeneral(DataLattice.GramMat, EXT);
  TheCenter:=CP.Center;
  CharPair:=CharacterizingPair(DataLattice.GramMat, TheCenter);
  return rec(Qmat:=CharPair[1], EXText:=EXText);
...
# We extend here by an invariant basis (which has to be a Z basis of Z^n)
Lattice_Stabilizer_Method9_Pair:=function(DataLattice, EXT, ThePoint)
  local RecMat, GRPbig, ListMatGens, eGen, eBigMat, eMat, GRPret;
  RecMat:=GetScalMat_Method9(DataLattice, EXT, ThePoint);
  GRPbig:=LinPolytope_Automorphism_GramMat(RecMat.EXText, RecMat.Qmat);
  Print("|GRPbig| = ", Order(GRPbig), "\n");
  ListMatGens:=[];
  for eGen in GeneratorsOfGroup(GRPbig)
...
# We use the AUTOM / ISOM program for the computation.
Lattice_Stabilizer_Method6:=function(DataLattice, EXT, TheCenter)
...
# In other cases, it may or may not work
Lattice_Stabilizer_Method0:=function(DataLattice, EXT, GRP)
...
StabilizerOfDelaunay:=function(DataLattice, EXT)
  ...
  if Length(EXT)<LimitEfficiencyNauty then
    REP:=Lattice_Stabilizer_Method0(DataLattice, EXT, GRP);
  else
    REP:=fail;
  fi;
  if REP<>fail then
    ...
    if Length(EXT)<LimitEfficiencyNauty then
      if DataLattice.n <= 10 then
        TheReply:=Lattice_Stabilizer_Method2(DataLattice, TheCenter);
      elif Order(GRP) < 50000 then
        TheReply:=Lattice_Stabilizer_Method4(DataLattice, EXT, GRP);
      elif LFactors<>"undefined" and Length(LFactors) > 2 then
        TheReply:=Lattice_Stabilizer_Method3(DataLattice, TheCenter);
      else
        TheReply:=Lattice_Stabilizer_Method6(DataLattice, EXT, TheCenter);
      fi;
    else
      if DataLattice.n <= 10 then
        TheReply:=Lattice_Stabilizer_Method2(DataLattice, TheCenter);
...
Lattice_Equivalence_Method0:=function(DataLattice, EXT1, EXT2, Delaunay1Stab, ePerm)
  local eBigMat;
...
Lattice_Equivalence_Method1:=function(DataLattice, EXT1, EXT2, Delaunay1Stab, ePerm)
...
Lattice_Equivalence_Method2:=function(DataLattice, ThePoint1, ThePoint2)
...
# of stabilizer,
# It uses only the centers.
Lattice_Equivalence_Method3:=function(DataLattice, ThePoint1, ThePoint2)
  ...
  Print("Begin of Lattice_Equivalence_Method3\n");
  n:=DataLattice.n;
  RedPointFirst1:=ThePoint1{[2..n+1]};
  RedPointFirst2:=ThePoint2{[2..n+1]};
  RedPointSecond1:=VectorMod1(RedPointFirst1);
...
Lattice_Equivalence_Method4:=function(DataLattice, EXT1, EXT2, GRP1, ePerm)
  local eElt, eBigMat;
  for eElt in GRP1
  do
...
Lattice_Equivalence_Method7:=function(DataLattice, EXT1, EXT2, GRP1, ePerm)
...
# The Invariant basis has to be invariant, but does not need to Z-span Z^n
Lattice_Equivalence_Method8:=function(DataLattice, EXT1, EXT2)
...
# The InvariantBasis has to Z-spanning Z^n
Lattice_Equivalence_Method9:=function(DataLattice, EXT1, EXT2)
  local test, RecMat1, RecMat2, eBigMat;
  RecMat1:=GetScalMat_Method8(DataLattice, EXT1);
  RecMat2:=GetScalMat_Method8(DataLattice, EXT2);
  test:=LinPolytope_Isomorphism_GramMat(RecMat1.EXText, RecMat1.Qmat, RecMat2.EXText, RecMat2.Qmat);
  Print("We have test\n");
  ...
  eBigMat:=FindTransformation(RecMat1.EXText, RecMat2.EXText, test);
  if IsIntegralMat(FuncExplodeBigMatrix(eBigMat).MatrixTransformation)=false then
      Error("The matrix should be integral");
  fi;
  return eBigMat;
end;
...
# The InvariantBasis has to Z-spanning Z^n
Lattice_Equivalence_Method9_Pair:=function(DataLattice, EXT1, ThePoint1, EXT2, ThePoint2)
  local test, RecMat1, RecMat2, eBigMat;
  RecMat1:=GetScalMat_Method9(DataLattice, EXT1, ThePoint1);
  RecMat2:=GetScalMat_Method9(DataLattice, EXT2, ThePoint2);
  test:=LinPolytope_Isomorphism_GramMat(RecMat1.EXText, RecMat1.Qmat, RecMat2.EXText, RecMat2.Qmat);
  Print("We have test\n");
  if test=false then
    return false;
...
# isometry group
Lattice_Equivalence_Method5:=function(DataLattice, EXT1, EXT2, Delaunay1Stab, ePerm)
  local u, eBigMat;
...
Lattice_Equivalence_Method6:=function(DataLattice, EXT1, EXT2, TheCenter1, TheCenter2)
...
#  Print("  ListFactors=", LFactors1);
  Print("\n");
  if Length(EXT1)<LimitEfficiencyNauty then
    REP:=Lattice_Equivalence_Method0(DataLattice, EXT1, EXT2, Delaunay1Stab, ePerm);
  ...
  if REP<>fail then
    ...
    if DataLattice.n<=7 then
      return Lattice_Equivalence_Method2(DataLattice, TheCenter1, TheCenter2);
    fi;
    if Length(EXT1)<LimitEfficiencyNauty then
      if Order(GRP1)<=50000 then
        return Lattice_Equivalence_Method4(DataLattice, EXT1, EXT2, GRP1, ePerm);
      fi;
    fi;
    if LFactors1<>"undefined" and Length(LFactors1)>2 then
      return Lattice_Equivalence_Method3(DataLattice, TheCenter1, TheCenter2);
    fi;
    if TheQuot< 100 then
      return Lattice_Equivalence_Method5(DataLattice, EXT1, EXT2, Delaunay1Stab, ePerm);
    fi;
    return Lattice_Equivalence_Method6(DataLattice, EXT1, EXT2, TheCenter1, TheCenter2);
  fi;
end;
...
#    Print("After call to FindAdjacentDelaunayPolytope\n");
  ...
  FuncEquiv:=function(EXT1, EXT2, ThePoint1, ThePoint2)
    return Lattice_Equivalence_Method6(DataLattice, EXT1, EXT2, ThePoint1, ThePoint2);
  end;
  FuncAutom:=function(EXT, ThePoint)
    local GRP1, RedPoint, MatrixGens, eMat, eTrans, GRPret;
    GRP1:=Lattice_Stabilizer_Method6(DataLattice, EXT, ThePoint);
  ...
  Print("|TheInvariantBase|=", Length(TheInvariantBase), " det=", AbsInt(DeterminantMat(BaseIntMat(TheInvariantBase))), "\n");
  MatrGens:=GeneratorsOfGroup(PointGRP);
  WorkingAffineBasis:=RecSVR.WorkingAffineBasis;
  PermGRP:=Group(ListPermGens);
  phi:=GroupHomomorphismByImagesNC(PermGRP, PointGRP, ListPermGens, MatrGens);
  DataLattice:=rec(
    Saving:=IsSaving,
...
DelaunayDescriptionStandardKernel:=function(TheGramMat, PointGRP, ThePrefix, IsSaving, KillingDelaunay, KillingAdjacency, MaximalMemorySave)
  ...
  if TheReply="all was ok" then
    return rec(success:=true, DelCO:=DLPD.DelaunayDatabase.FuncReturnCompleteDescription());
  else
    return rec(success:=false, error:=TheReply);
  fi;
end;

DelaunayDescriptionStandardKernelInvFamily:=function(TheGramMat, PointGRP, InvariantBasis, ThePrefix, IsSaving, KillingDelaunay, KillingAdjacency, MaximalMemorySave)
  local RecSVR, DLPD, TheReply;
  Print("Before GetRecSVR_FromInvFamily |InvariantBasis|=", Length(InvariantBasis), "\n");
  RecSVR:=GetRecSVR_FromInvFamily(TheGramMat, PointGRP, InvariantBasis);
  Print("We have RecSVR\n");
  DLPD:=ForKernel_DataLatticePolyhedralDatabase(TheGramMat, PointGRP, ThePrefix, IsSaving, MaximalMemorySave, RecSVR);
  DLPD.DataLattice.KillingDelaunay:=KillingDelaunay;
  DLPD.DataLattice.KillingAdjacency:=KillingAdjacency;
  TheReply:=ComputeDelaunayDecomposition(DLPD.DataLattice, DLPD.DataPolyhedral, DLPD.DelaunayDatabase);
...
Periodic_Stabilizer_Method0:=Lattice_Stabilizer_Method0;

Periodic_Equivalence_Method0:=Lattice_Equivalence_Method0;



Periodic_Equivalence_Method2:=function(DataLattice, TheCenter1, TheCenter2)
  local n, RedCenterFirst1, RedCenterFirst2, RedCenter1, RedCenter2, FuncRawImage, FuncActionMod1, eElt, eMat, eBigMat, eTrans, PermGRP, nbGen, f, hom, hom2;
  n:=DataLattice.n;
...
Periodic_Stabilizer_Method2:=function(DataLattice, TheCenter)
  local n, RedCenterFirst, RedCenter, FuncRawImage, FuncActionMod1, PermGRP, PermStab, ListMatStab, GRP, nbGen, f, hom, hom2;
  n:=DataLattice.n;
...
GetHigherRepresentative:=function(eMat, ListCoset)
    local n, ListCosetRed, eCos, iCos, fCos, eTrans, eBigMat, ListCosetImg, ListPos;
    n:=Length(eMat);
    ListCosetRed:=List(ListCoset, x->x{[2..n+1]});
    eCos:=ListCosetRed[1];
    for iCos in [1..Length(ListCosetRed)]
    do
        fCos:=ListCosetRed[iCos];
        eTrans:=fCos - eCos * eMat;
        eBigMat:=FuncCreateBigMatrix(eTrans, eMat);
        ListCosetImg:=List(ListCoset, x->x*eBigMat);
        ListPos:=List(ListCosetImg, x->Position(ListCoset, PeriodicVectorMod1(x)));
        if ListPos[1]<>iCos then
            Error("Conceptual error");
        fi;
...
Periodic_Equivalence_Method3:=function(DataLattice, TheCenter1, TheCenter2)
  local n, RedCenterFirst1, RedCenterFirst2, RedCenter1, RedCenter2, RedCenterPrev1, RedCenterPW1, RedCenterPW2, ReductionModPW, FuncRawImage, FuncActionModPW, eElt, eMat, eBigMat, eBigMatSearch, eTrans, GrpStab, OneDen, GroupDen, LFactors1, LSpaceFactors1, i, PermGRP, ListSequenceDenominator, eDen, nbGen, f, hom, hom2, FuncRawImage_V1;
  n:=DataLattice.n;
  RedCenter1:=PeriodicVectorMod1(TheCenter1);
  RedCenter2:=PeriodicVectorMod1(TheCenter2);
  ...
  hom:=GroupHomomorphismByImagesNC(f, PermGRP, GeneratorsOfGroup(f), GeneratorsOfGroup(PermGRP));
  hom2:=GroupHomomorphismByImagesNC(f, DataLattice.MatrixGRP, GeneratorsOfGroup(f), GeneratorsOfGroup(DataLattice.MatrixGRP));
  FuncRawImage_V1:=function(eElt)
      return Image(hom2, PreImagesRepresentative(hom, eElt));
  end;
  FuncRawImage:=function(eElt)
    local eMat;
    eMat:=Image(DataLattice.phi, eElt);
    return GetHigherRepresentative(eMat, DataLattice.ListCoset);
  end;
  #
  RedCenterPrev1:=ShallowCopy(RedCenter1);
  GrpStab:=ShallowCopy(DataLattice.PermGRP);
  eBigMatSearch:=IdentityMat(n+1);
  ...
  do
    ...
    eBigMat:=FuncRawImage(eElt);
    eBigMatSearch:=eBigMatSearch * eBigMat;
    RedCenterPrev1:=PeriodicVectorMod1(RedCenterPrev1 * eBigMat);
    if RedCenterPrev1=RedCenter2 then
      eMat:=FuncExplodeBigMatrix(eBigMatSearch).MatrixTransformation;
      eTrans:=TheCenter2{[2..n+1]} - TheCenter1{[2..n+1]} * eMat;
      return FuncCreateBigMatrix(eTrans, eMat);
    fi;
    GrpStab:=Stabilizer(GrpStab, RedCenterPW2, FuncActionModPW);
    GrpStab:=Group(SmallGeneratingSet(GrpStab));
  od;
end;

Periodic_Stabilizer_Method3:=function(DataLattice, TheCenter)
  local n, RedCenterFirst, RedCenter, GroupDen, LFactors, LSpaceFactors, ListSequenceDenominator, i, OneDen, PermStab, TheOrd, FuncRawImage, eDen, ReductionModPW, FuncActionModPW, RedCenterPW, eComp, GRP, ListMatStab, nbGen, f, hom, hom2;
  n:=DataLattice.n;
  RedCenterFirst:=TheCenter{[2..n+1]};
  RedCenter:=Concatenation([1], VectorMod1(RedCenterFirst));
  GroupDen:=DataLattice.GroupDen;
  ...
  #
  Print("ListSequenceDenominator=", ListSequenceDenominator ,"\n");
  for eDen in ListSequenceDenominator
  do
    ReductionModPW:=function(eVect)
      return Concatenation([1], List(eVect{[2..n+1]}, x->FractionModPW(x,eDen)));
    ...
    RedCenterPW:=ReductionModPW(TheCenter);
...
Periodic_Equivalence_Method4:=Lattice_Equivalence_Method4;

Periodic_Stabilizer_Method4:=Lattice_Stabilizer_Method4;


Periodic_Equivalence_Method9:=Lattice_Equivalence_Method9;

Periodic_Stabilizer_Method9:=Lattice_Stabilizer_Method9;


Periodic_Equivalence_Method9_Pair:=Lattice_Equivalence_Method9_Pair;

Periodic_Stabilizer_Method9_Pair:=Lattice_Stabilizer_Method9_Pair;



Periodic_Stabilizer_Method7:=function(DataLattice, EXT, GRP)
  local n, ListBigMatGens, ListMatGens, eIso, eGen, eBigMat, eMat, GRPmatr_big, GRPmatr, eBasis, FirstCol, NSP, eBasisRed, eNSP, eVect, eVectRed, ListMatSpace, eMatSpace, GRPspace, LattToStab, eStab, ListMatGensRet, ListPermGensRet, eTrans, ePermRet, GRPret, TheSize;
  n:=DataLattice.n;
  Error("This code is buggy. Try D5+ to see it");
  ListBigMatGens:=[];
  ListMatGens:=[];
  eIso:=Isobarycenter(EXT);
  for eGen in GeneratorsOfGroup(GRP)
  do
      eBigMat:=FindTransformation(EXT, EXT, eGen);
      eMat:=FuncExplodeBigMatrix(eBigMat).MatrixTransformation;
      Add(ListBigMatGens, eBigMat);
      Add(ListMatGens, eMat);
  od;
  GRPmatr_big:=Group(ListBigMatGens);
  GRPmatr    :=Group(ListMatGens);
  eBasis:=GetZbasis(EXT);
  FirstCol:=List(eBasis, x->x[1]);
  NSP:=NullspaceIntMat(TransposedMat([FirstCol]));
  eBasisRed:=[];
  for eNSP in NSP
  do
      eVect:=eNSP * eBasis;
      eVectRed:=eVect{[2..n+1]};
      Add(eBasisRed, eVectRed);
  od;
  ListMatSpace:=[];
  for eMat in ListMatGens
  do
      eMatSpace:=eBasisRed * eMat * Inverse(eBasisRed);
      if IsIntegralMat(eMatSpace)=false then
          Error("Thinking error here");
      fi;
      Add(ListMatSpace, eMatSpace);
  od;
  GRPspace:=Group(ListMatSpace);
  LattToStab:=RemoveFractionMatrix(Inverse(eBasisRed));
  Print("We have LattToStab\n");
  ...
  do
    eMat:=Inverse(eBasisRed) * eGen * eBasisRed;
    if IsIntegralMat(eMat)=false then
      Error("eMat should be integral");
    fi;
    eTrans:=eIso - eIso * eMat;
    eBigMat:=FuncCreateBigMatrix(eTrans, eMat);
    ePermRet:=PermList(List(EXT, x->Position(EXT, x*eBigMat)));
    Add(ListMatGensRet, eMat);
    Add(ListPermGensRet, ePermRet);
  od;
  GRPret:=PersoGroupMatrix(ListMatGensRet, n);
  TheSize:=Order(Group(ListPermGensRet));
  SetSize(GRPret, TheSize);
  return GRPret;
end;

Periodic_Equivalence_Method6:=function(DataLattice, EXT1, EXT2, ThePoint1, ThePoint2)
    ...
    do
        ...
        TheEquiv:=Lattice_Equivalence_Method6(DataLattice, EXT1, imgEXT2, ThePoint1, imgThePoint2);
...
Periodic_Stabilizer_Method6:=function(DataLattice, EXT, ThePoint)
  local TheStab, ListMatGens, nbMatch, eBigMat, imgThePoint, imgEXT, TheEquiv, eAddBigMat, GRP;
  TheStab:=Lattice_Stabilizer_Method6(DataLattice, EXT, ThePoint);
  ...
  do
    ...
    imgEXT:=EXT*eBigMat;
    TheEquiv:=Lattice_Equivalence_Method6(DataLattice, EXT, imgEXT, ThePoint, imgThePoint);
    if TheEquiv<>false then
...
Periodic_TestEquivalenceOfDelaunay:=function(DataLattice, EXT1, EXT2, Delaunay1Stab)
  ...
  ePerm:=PermList(test);
  GRP1:=Delaunay1Stab.PrivateInfo.GRP;
  Print("Periodic_TestEquivalenceOfDelaunay |GRP1|=", Order(GRP1), "\n");
  GroupDen:=DataLattice.GroupDen;
  LFactors1:=FactorsInt(LcmInt(GroupDen, ListFactors(TheCenter1))/GroupDen);
  REP:=Periodic_Equivalence_Method0(DataLattice, EXT1, EXT2, Delaunay1Stab, ePerm);
  if REP<>fail then
    return REP;
  else
    if DataLattice.IsMethod6ok then
      return Periodic_Equivalence_Method6(DataLattice, EXT1, EXT2, TheCenter1, TheCenter2);
    fi;
    if DataLattice.n<=6 then
      return Periodic_Equivalence_Method2(DataLattice, TheCenter1, TheCenter2);
    fi;
    if Order(GRP1)<=50000 then
      return Periodic_Equivalence_Method4(DataLattice, EXT1, EXT2, GRP1, ePerm);
    fi;
    if Length(LFactors1)>2 then
      return Periodic_Equivalence_Method3(DataLattice, TheCenter1, TheCenter2);
    fi;
    return Periodic_Equivalence_Method2(DataLattice, TheCenter1, TheCenter2);
  fi;
end;
...
Periodic_StabilizerOfDelaunay:=function(DataLattice, EXT)
  local CP, TheCenter, n, DM, GRP, EasyCase, REP, TheReply, LFactors, GroupDen, HLR, DoMethod7, DoMethod9;
  CP:=CenterRadiusDelaunayPolytopeGeneral(DataLattice.GramMat, EXT);
  TheCenter:=CP.Center;
  DM:=DistanceMatrixDelaunay(DataLattice.GramMat, EXT);
  GRP:=AutomorphismGroupEdgeColoredGraph(DM);
  Print("Periodic_StabilizerOfDelaunay |GRP|=", Order(GRP), "\n");
  GroupDen:=DataLattice.GroupDen;
  LFactors:=FactorsInt(LcmInt(GroupDen, ListFactors(TheCenter))/GroupDen);
  REP:=Periodic_Stabilizer_Method0(DataLattice, EXT, GRP);
  if REP<>fail then
    EasyCase:=true;
    TheReply:=REP;
  elif true then
    EasyCase:=false;
    DoMethod7:=false;
    DoMethod9:=false; # The Method9 is actually bugged. It uses only the Gram matrix for determining the
                      # preserved elements. Since we cannot encode the cosets in the formulation of the
                      # automorphism problem. And there is no immediate way to resolve the problem.
    if DoMethod7 then
      Print("Perioddic_StabilizerOfDelaunay, Method7\n");
      TheReply:=Periodic_Stabilizer_Method7(DataLattice, EXT, GRP);
    elif DoMethod9 then
      Print("Perioddic_StabilizerOfDelaunay, Method9\n");
      TheReply:=Periodic_Stabilizer_Method9(DataLattice, EXT);
    elif DataLattice.IsMethod6ok then
      Print("Perioddic_StabilizerOfDelaunay, Method6\n");
      TheReply:=Periodic_Stabilizer_Method6(DataLattice, EXT, TheCenter);
    elif DataLattice.n<=6 then
      Print("Perioddic_StabilizerOfDelaunay, Method2\n");
      TheReply:=Periodic_Stabilizer_Method2(DataLattice, TheCenter);
    elif Order(GRP)< 500000 then
      Print("Perioddic_StabilizerOfDelaunay, Method4\n");
      TheReply:=Periodic_Stabilizer_Method4(DataLattice, EXT, GRP);
    elif Order(GRP)>10000000 then
      Print("Perioddic_StabilizerOfDelaunay, Method2\n");
      TheReply:=Periodic_Stabilizer_Method2(DataLattice, TheCenter);
    elif Length(LFactors)>1 then
      Print("Perioddic_StabilizerOfDelaunay, Method3\n");
      TheReply:=Periodic_Stabilizer_Method3(DataLattice, TheCenter);
    else
      Print("Perioddic_StabilizerOfDelaunay, Method3\n");
        if Length(EXT) = 14 and Order(GRP) = 645120 then
            TheReply:=Periodic_Stabilizer_Method9(DataLattice, EXT);
...
CrystallographicGroup:=function(GramMat, ListCoset)
  local n, eCos, GetTranslations, Method1, Method2;
  n:=Length(GramMat);
  for eCos in ListCoset
  do
  ...
  od;
  # Simple iteration over the elements
  ...
  Method1:=function()
    local ListCrystGens, ListPointGens, ProvPtGRP, FuncInsert, PtGRP, iCos, eMat, eTrans, eBigMat, IsOK, pos;
    ListCrystGens:=GetTranslations();
    ListPointGens:=[];
  ...
  # First determination of the stabilizer of 0.
  # Then iterating over the cosets for finding the representative.
  Method2:=function()
      local PtGRP, InvariantBasis, ListMatrGens, ListMatrGensExt, ListPermGens, eTrans, eGen, ePerm, eBigMat, PtGRP_ext, PtGRP_perm, phi, phi_ext, eCos, eO, eSetCoset, ListPermGenCoset, iGen, ePermCoset, FirstStab, ListCrystGens, ListGRP_coset, eGRP_coset, eRepr, eReprMat, TotalOrbit;
      PtGRP:=ArithmeticAutomorphismGroup([GramMat]);
      Print("CrystallographicGroup |PtGRP|=", Order(PtGRP), "\n");
      InvariantBasis:=__ExtractInvariantZBasisShortVectorNoGroup(GramMat);
      ListMatrGens:=GeneratorsOfGroup(PtGRP);
      ListMatrGensExt:=[];
      ListPermGens:=[];
      eTrans:=ListWithIdenticalEntries(n, 0);
      for eGen in ListMatrGens
      do
          ePerm:=PermList(List(InvariantBasis, x->Position(InvariantBasis, x*eGen)));
          eBigMat:=FuncCreateBigMatrix(eTrans, eGen);
          Add(ListPermGens, ePerm);
          Add(ListMatrGensExt, eBigMat);
      od;
      PtGRP_ext:=Group(ListMatrGensExt);
      PtGRP_perm:=Group(ListPermGens);
      phi:=GroupHomomorphismByImagesNC(PtGRP_perm, PtGRP, ListPermGens, ListMatrGens);
      phi_ext:=GroupHomomorphismByImagesNC(PtGRP_perm, PtGRP_ext, ListPermGens, ListMatrGensExt);
      Print("|PtGRP_perm|=", Order(PtGRP_perm), "\n");
      TotalOrbit:=[];
      for eCos in ListCoset
      do
          if Position(TotalOrbit, eCos)=fail then
              eO:=Orbit(PtGRP_ext, eCos, PeriodicOnClassesMod1);
              Append(TotalOrbit, eO);
          fi;
      od;
      eSetCoset:=Set(List(ListCoset, x->Position(TotalOrbit, x)));
      ListPermGenCoset:=[];
      for iGen in [1..Length(ListMatrGens)]
      do
          eBigMat:=ListMatrGensExt[iGen];
          ePermCoset:=PermList(List(TotalOrbit, x->Position(TotalOrbit, PeriodicOnClassesMod1(x, eBigMat))));
          Add(ListPermGenCoset, ePermCoset);
      od;
      FirstStab:=Stabilizer(PtGRP_perm, eSetCoset, ListPermGens, ListPermGenCoset, OnSets);
      ListCrystGens:=GetTranslations();
      for eGen in GeneratorsOfGroup(FirstStab)
      do
          Add(ListCrystGens, Image(phi_ext, eGen));
      od;
      Print("|FirstStab|=", Order(FirstStab), "\n");
      ListGRP_coset:=RightCosets(PtGRP_perm, FirstStab);
      for eGRP_coset in ListGRP_coset
      do
          eRepr:=Representative(eGRP_coset);
          eReprMat:=Image(phi, eRepr);
          eBigMat:=GetHigherRepresentative(eReprMat, ListCoset);
          if eBigMat<>fail then
              Add(ListCrystGens, eBigMat);
          fi;
      od;
      return PersoGroupMatrix(ListCrystGens, n+1);
...
ForKernel_Periodic_DataLatticePolyhedralDatabase:=function(GramMat, MatrixGRP, ListCoset, ThePrefix, IsSaving)
  local n, ListMatrGens, ListPointGens, PointGRP, ListPermGens, PermGRP, phi, FuncRawImage, TestBelonging, DataLattice, FindAdjacentDelaunay, FindDelaunay, eCos, IsBankSave, IsRespawn, FuncStabilizer, FuncIsomorphy, DataPolyhedral, DelaunayDatabase, PathTMP, PathSAVE, PathPermanent, TheReply, FuncInvariant, eBigMat, eMat, FuncEquiv, FuncAutom, GroupDen, MyOption, FuncLatticeAutomorphism, FuncLatticeIsomorphism, TheInvariantBase, WorkingAffineBasis, TheComp, IsMethod6ok, TheZero, ZeroStabilizer, GramMatGrp, ListPermCosets, GroupCoset, eList, eGen, ListCosetRed, FindClosestElement, KillingDelaunay, KillingAdjacency, DoMethod9;
  n:=Length(GramMat);
  ...
  DoMethod9:=false;
  FuncEquiv:=function(EXT1, EXT2, ThePoint1, ThePoint2)
    local LFactors1, TheAns;
    LFactors1:=FactorsInt(LcmInt(GroupDen, ListFactors(ThePoint1))/GroupDen);
    Print("FuncEquiv : LFactors1=", LFactors1, "\n");
    if IsMethod6ok=true then
      Print("Using Periodic_Equivalence_Method6\n");
      TheAns:=Periodic_Equivalence_Method6(DataLattice, EXT1, EXT2, ThePoint1, ThePoint2);
    elif n<=6 then
      Print("Using Periodic_Equivalence_Method2\n");
      TheAns:=Periodic_Equivalence_Method2(DataLattice, ThePoint1, ThePoint2);
    elif DoMethod9 then
      Print("Using Periodic_Equivalence_Method9\n");
      TheAns:=Periodic_Equivalence_Method9_Pair(DataLattice, EXT1, ThePoint1, EXT2, ThePoint2);
    elif Length(LFactors1)>1 then
      Print("Using Periodic_Equivalence_Method3\n");
      TheAns:=Periodic_Equivalence_Method3(DataLattice, ThePoint1, ThePoint2);
    else
      Print("Using Periodic_Equivalence_Method2\n");
      TheAns:=Periodic_Equivalence_Method2(DataLattice, ThePoint1, ThePoint2);
    fi;
    if TheAns=false then
      return false;
    fi;
    if ThePoint2<>ThePoint1*TheAns then
        Error("Inconsistency in ThePoint1/2");
    fi;
    if First(EXT1, x->Position(EXT2, x*TheAns)=fail)<>fail then
        Error("EXT1 not mapped to EXT2");
    fi;
    return TheAns;
  end;
  FuncAutom:=function(EXT, ThePoint)
    ...
    Print("FuncAutom : LFactors=", LFactors, "\n");
    if IsMethod6ok=true then
      Print("Using Periodic_Stabilizer_Method6\n");
      TheGRP:=Periodic_Stabilizer_Method6(DataLattice, EXT, ThePoint);
    elif n<=6 then
      Print("Using Periodic_Stabilizer_Method2\n");
      TheGRP:=Periodic_Stabilizer_Method2(DataLattice, ThePoint);
    elif DoMethod9 then
      Print("Using Periodic_Stabilizer_Method9\n");
      TheGRP:=Periodic_Stabilizer_Method9_Pair(DataLattice, EXT, ThePoint);
    elif Length(LFactors)>1 then
      Print("Using Periodic_Stabilizer_Method3\n");
      TheGRP:=Periodic_Stabilizer_Method3(DataLattice, ThePoint);
    else
      Print("Using Periodic_Stabilizer_Method2\n");
      TheGRP:=Periodic_Stabilizer_Method2(DataLattice, ThePoint);
    fi;
    ...
    do
      ...
      eTrans:=RedPoint-RedPoint*eMat;
      eBigMat:=FuncCreateBigMatrix(eTrans, eMat);
      if First(EXT, x->Position(EXT, x*eBigMat)=fail)<>fail then
          Print("EXT is not preserved");
      fi;
      Add(MatrixGens, eBigMat);
    od;
    GRPret:=Group(MatrixGens);
...
#    return ArithmeticEquivalenceMatrixFamily_HackSouvignier(MyOption, ListMat1, SVR1, AffBas1, ListMat2, SVR2, AffBas2);
  ...
  TheZero[1]:=1;
  ZeroStabilizer:=Periodic_Stabilizer_Method2(DataLattice, TheZero);
...
TestEquivalenceOfDelaunayHighDimMethod6:=function(DataLattice, EXT1, EXT2, Delaunay1Stab)
  ...
  CP2:=CenterRadiusDelaunayPolytopeGeneral(DataLattice.GramMat, EXT2);
  TheCenter2:=CP2.Center;
  eEquiv:=Lattice_Equivalence_Method6(DataLattice, EXT1, EXT2, TheCenter1, TheCenter2);
...
PROC_GetNeighborhood:=function(eRecordFirst, DelaunayDatabase)
    local EXTdelaunay, ListCenter, ListDone, ListRecord, FuncInsert, DoIncrement, GetAllAddedVertices, PrintStatistics, ListGeneration, iGeneration;
    EXTdelaunay:=DelaunayDatabase.FuncDelaunayGetEXT(eRecordFirst.iDelaunay) * eRecordFirst.eEquiv;
    ...
    FuncInsert:=function(NewRecord)
      local eCent;
      eCent:=Isobarycenter(DelaunayDatabase.FuncDelaunayGetEXT(NewRecord.iDelaunay)*NewRecord.eBigMat);
    ...
    DoIncrement:=function()
      ...
      do
        if ListDone[iRecord]=0 then
          ListDone[iRecord]:=1;
          eRecord:=ListRecord[iRecord];
          TheAdjacencies:=DelaunayDatabase.FuncDelaunayGetAdjacencies(eRecord.iDelaunay);
          TheStab:=DelaunayDatabase.FuncDelaunayGetGroup(eRecord.iDelaunay);
    ...
    GetAllAddedVertices:=function()
      ...
      do
        TheEXT:=DelaunayDatabase.FuncDelaunayGetEXT(eRecord.iDelaunay)*eRecord.eBigMat;
...
PROC_GetContainingCells:=function(eVert, DelaunayDatabase)
    local n, eRec, EXT, eEXT, GetOneShift, ListVect, TheRel;
    n:=Length(eVert);
    ...
    GetOneShift:=function(eRec)
      ...
      EXT:=DelaunayDatabase.FuncDelaunayGetEXT(eRec.iDelaunay)*eRec.eBigMat;
      TheAdjacencies:=DelaunayDatabase.FuncDelaunayGetAdjacencies(eRec.iDelaunay);
      TheStab:=DelaunayDatabase.FuncDelaunayGetGroup(eRec.iDelaunay);
      for eAdj in TheAdjacencies
      do
        O:=OrbitWithAction(TheStab.PermutationStabilizer, eAdj.eInc, OnSets);
        for ePerm in O.ListElement
...
PROC_ApproximateInfinity:=function(n_iter, DelaunayDatabase)
    local n, nbDel, GetFullRec, ListNeighbors, iDel, RawListAdj, TheAdjacencies, TheStab, eAdj, O, ePerm, eMatrix, eRec, ListRec, FuncInsert, pos, i_iter, n_rec, iRec, newRec, EXT, EXTdupli, SetCent;
    n:=Length(DelaunayDatabase.FuncDelaunayGetEXT(1)[1]) - 1;
    nbDel:=DelaunayDatabase.FuncDelaunayGetNumber();
    GetFullRec:=function(eRec)
        local eIso;
        eIso:=Isobarycenter(DelaunayDatabase.FuncDelaunayGetEXT(eRec.iDelaunay) * eRec.eBigMat);
        return rec(iDelaunay:=eRec.iDelaunay, eBigMat:=eRec.eBigMat, eIso:=eIso);
    end;
    ListNeighbors:=[];
    for iDel in [1..nbDel]
    do
        RawListAdj:=[];
        TheAdjacencies:=DelaunayDatabase.FuncDelaunayGetAdjacencies(iDel);
        TheStab:=DelaunayDatabase.FuncDelaunayGetGroup(iDel);
        for eAdj in TheAdjacencies
        do
            O:=OrbitWithAction(TheStab.PermutationStabilizer, eAdj.eInc, OnSets);
            ...
            do
                eMatrix:=Image(TheStab.PhiPermMat, ePerm);
                eRec:=rec(iDelaunay:=eAdj.iDelaunay, eBigMat:=eAdj.eBigMat * eMatrix);
                Add(RawListAdj, GetFullRec(eRec));
            od;
        od;
        Add(ListNeighbors, RawListAdj);
    od;
    ListRec:=[];
    SetCent:=Set([]);
    FuncInsert:=function(eRec)
        if eRec.eIso in SetCent then
            return;
        fi;
        Add(ListRec, eRec);
        AddSet(SetCent, eRec.eIso);
    end;
    FuncInsert(GetFullRec(rec(iDelaunay:=1, eBigMat:=IdentityMat(n+1))));
    pos:=0;
    for i_iter in [1..n_iter]
    do
        n_rec:=Length(ListRec);
        for iRec in [pos+1..n_rec]
        do
            eRec:=ListRec[iRec];
            for eAdj in ListNeighbors[eRec.iDelaunay]
            do
                newRec:=rec(iDelaunay:=eAdj.iDelaunay, eBigMat:=eAdj.eBigMat * eRec.eBigMat, eIso:=eAdj.eIso * eRec.eBigMat);
                FuncInsert(newRec);
            od;
        od;
        pos:=n_rec;
...
PROC_GetAllContainingCells:=function(eVert, DelaunayDatabase)
    ...
    FuncInsert:=function(eRec)
      ...
      EXT:=DelaunayDatabase.FuncDelaunayGetEXT(iDelaunay)*eBigMat;
      eIso:=Isobarycenter(EXT);
      for fRec in ListRec
      do
    ...
    FuncInsert(PROC_GetContainingCells(eVert, DelaunayDatabase));
    while(true)
    do
...
#  PointGRP:=ArithmeticAutomorphismMatrixFamily_HackSouvignier_V2("", [TheGramMat], InvariantBasis);
  ...
  GetNeighborhood:=function(EXTdelaunay)
      ...
      return PROC_GetNeighborhood(eRecordFirst, DLPD.DelaunayDatabase);
  end;
  GetContainingCell:=function(eVert)
      return PROC_GetContainingCells(eVert, DLPD.DelaunayDatabase);
  end;
  GetAllContainingCells:=function(eVert)
      return PROC_GetAllContainingCells(eVert, DLPD.DelaunayDatabase);
  end;
  ApproximateInfinity:=function(n_iter)
      return PROC_ApproximateInfinity(n_iter, DLPD.DelaunayDatabase);
  end;
  ...
  GetQuantization_ConwaySloane:=function()
      local TheDet, SecMoment, eDim, eExpo;
      SecMoment:=GetQuantization();
      TheDet:=DeterminantMat(TheGramMat) + 0.0;
  ...
  GetOrbitVoronoiVertices:=function()
      local ListOrbit, nbDel, iDelaunay, TheStab, TheEXT, n_ext, O, eO;
      ListOrbit:=[];
      nbDel:=DLPD.DelaunayDatabase.FuncDelaunayGetNumber();
      for iDelaunay in [1..nbDel]
      do
          ...
          TheEXT:=DLPD.DelaunayDatabase.FuncDelaunayGetEXT(iDelaunay);
          n_ext:=Length(TheEXT);
          O:=Orbits(TheStab.PermutationStabilizer, [1..n_ext], OnPoints);
          for eO in O
          do
              Add(ListOrbit, eO[1]); # Unused right now
          od;
      od;
      return ListOrbit;
  end;
  GetVoronoiVertices:=function()
    local preFAC, preEXT, nbDel, iDelaunay, TheEXT, DDA, nbV, iExt, eAdj, eVect, O, CP, eEXT, eDiff, EXT, FAC, eCst, eFAC, GetGraph, GetCanonicalGraph, ListPermGenExt, ListPermGenFac, eGen, ePermFac, ePermExt, eListExt, eListFac, GroupFac, GroupExt, GetSubordinationInformation, GetFaceLattice;
    preFAC:=[];
    preEXT:=[];
    nbDel:=DLPD.DelaunayDatabase.FuncDelaunayGetNumber();
    for iDelaunay in [1..nbDel]
    ...
    for eGen in GeneratorsOfGroup(PointGRP)
    do
      eListFac:=List(preFAC, x->Position(preFAC, x*eGen));
      ePermFac:=PermList(eListFac);
      Add(ListPermGenFac, ePermFac);
      eListExt:=List(preEXT, x->Position(preEXT, x*eGen));
      ePermExt:=PermList(eListExt);
      Add(ListPermGenExt, ePermExt);
    od;
    GroupFac:=Group(ListPermGenFac);
    GroupExt:=Group(ListPermGenExt);
    GetGraph:=function()
...
# Octahedron ABCDEF
  ...
  GetListVolumePolytope:=function()
    local nbDel, ListVolumePolytope, iDelaunay, EXT, TheVol, TheSum, eNb, TheStab, ListNB;
    nbDel:=DLPD.DelaunayDatabase.FuncDelaunayGetNumber();
    ListVolumePolytope:=[];
    ListNB:=[];
    TheSum:=0;
    ...
    do
      ...
      Add(ListNB, eNb);
    od;
    Print("ListVolumePolytope=", ListVolumePolytope, " ListNB=", ListNB, "\n");
    if TheSum<>1 then
  ...
  return rec(GetDelaunayDescription:=GetDelaunayDescription,
             ...
             ApproximateInfinity:=ApproximateInfinity,
...
GetDelaunayDatabaseFromFullList:=function(FullList)
    local FuncDelaunayGetNumber, FuncDelaunayGetEXT, FuncDelaunayGetGroup, FuncDelaunayGetAdjacencies;
    FuncDelaunayGetNumber:=function()
        return Length(FullList);
    end;
    FuncDelaunayGetEXT:=function(iDelaunay)
        return FullList[iDelaunay].EXT;
    end;
    FuncDelaunayGetGroup:=function(iDelaunay)
        return FullList[iDelaunay].eStab;
    end;
    FuncDelaunayGetAdjacencies:=function(iDelaunay)
        return FullList[iDelaunay].ListAdj;
    end;
    return rec(FuncDelaunayGetNumber:=FuncDelaunayGetNumber,
               FuncDelaunayGetEXT:=FuncDelaunayGetEXT,
               FuncDelaunayGetGroup:=FuncDelaunayGetGroup,
               FuncDelaunayGetAdjacencies:=FuncDelaunayGetAdjacencies);
end;
...
Periodic_DelaunayComputationStandardFunctions:=function(U)
  local ListCosets, TheGramMat, n, AffineCrystGroup, DLPD, KillingDelaunay, KillingAdjacency, TheReply, ThePrefix, IsSaving, GetDelaunayDescription, GetVoronoiVertices, GetQuantization, GetQuantization_ConwaySloane, TheBoundary, LevelDoneBoundary, ComputeBoundary, GetBoundary, GetOrbitFoldHomology, CheckDelaunayTesselation, FlagFunctions, GetGroup, GetListVolumePolytope, GetInternationalTableGroupName, HasDelaunaySkeleton, ListSkeleton, GetVertexNeighbors, ComputeDelaunaySkeleton, CheckTotalVolume, GetFullListDelaunay;
  ...
  Print("We have AffineCrystGroup\n");
  ThePrefix:=Filename(POLYHEDRAL_tmpdir, "DualDesc/");
  Exec("mkdir -p ", ThePrefix);
  IsSaving:=false;
  LevelDoneBoundary:=-1;
  ...
  GetGroup:=function()
    local InvariantBase, ListPtGens, ListPermGens, ThePtGroup, ThePtGroup_perm;
    InvariantBase:=DLPD.DataLattice.InvariantBase;
    ListPtGens:=List(GeneratorsOfGroup(AffineCrystGroup), x->FuncExplodeBigMatrix(x).MatrixTransformation);
    ListPermGens:=List(ListPtGens, x->PermList(List(InvariantBase, y->Position(InvariantBase, y*x))));
    ThePtGroup:=Group(ListPtGens);
    ThePtGroup_perm:=Group(ListPermGens);
    return rec(AffineCrystGroup:=AffineCrystGroup, PointGroup:=ThePtGroup, PointGroupPerm:=ThePtGroup_perm);
  end;
  ...
  GetListVolumePolytope:=function()
    local nbDel, ListVolumePolytope, iDelaunay, EXT, TheVol, TheSum, eNb, TheOrd, TheStab, ListNB;
    TheOrd:=Order(GetGroup().PointGroupPerm);
    nbDel:=DLPD.DelaunayDatabase.FuncDelaunayGetNumber();
    ListVolumePolytope:=[];
    ListNB:=[];
    TheSum:=0;
    ...
    do
      ...
      TheStab:=DLPD.DelaunayDatabase.FuncDelaunayGetGroup(iDelaunay);
      TheVol:=VolumeComputationPolytope(EXT);
      eNb:=TheOrd/Order(TheStab.PermutationStabilizer);
      TheSum:=TheSum + eNb*TheVol;
      Add(ListVolumePolytope, TheVol);
      Add(ListNB, eNb);
    od;
  ...
  GetFullListDelaunay:=function()
    local ListOrbit, GetFullRecord, ListRepr, InsertPair, n_done, n_before, idx, eBigMat, eOrbit, eAdj, nBigMat, RecO, ePermRepr, eMatrRepr, PermGRP;
    ListOrbit:=GetDelaunayDescription();
    GetFullRecord:=function(iOrbit, eBigMat)
        local EXT, eCent;
        EXT:=ListOrbit[iOrbit].EXT * eBigMat;
        eCent:=Isobarycenter(EXT);
        return rec(iOrbit:=iOrbit, eBigMat:=eBigMat, eCent:=eCent);
    end;
    ListRepr:=[GetFullRecord(1, IdentityMat(n+1))];
    InsertPair:=function(iOrbit, eBigMat)
        local eRecord, eRepr, TheDiff;
        eRecord:=GetFullRecord(iOrbit, eBigMat);
        for eRepr in ListRepr
        do
            TheDiff:=eRecord.eCent - eRepr.eCent;
            if IsIntegralVector(TheDiff) then
                return;
            fi;
        od;
        Add(ListRepr, eRecord);
    end;
    n_done:=0;
    while(true)
    do
        n_before:=Length(ListRepr);
        if n_done = n_before then
            break;
        fi;
        for idx in [n_done+1..n_before]
        do
            eBigMat:=ListRepr[idx].eBigMat;
            eOrbit:=ListOrbit[ListRepr[idx].iOrbit];
            PermGRP:=eOrbit.TheStab.PermutationStabilizer;
            for eAdj in eOrbit.Adjacencies
            do
                RecO:=OrbitWithAction(PermGRP, eAdj.eInc, OnSets);
                for ePermRepr in RecO.ListElement
                do
                    eMatrRepr:=Image(eOrbit.TheStab.PhiPermMat, ePermRepr);
                    nBigMat:=eAdj.eBigMat * eMatrRepr * eBigMat;
                    InsertPair(eAdj.iDelaunay, nBigMat);
                od;
            od;
        od;
        n_done:=n_before;
    od;
    return List(ListRepr, x->ListOrbit[x.iOrbit].EXT * x.eigMat);
  end;
  ...
  return rec(GetVoronoiVertices:=GetVoronoiVertices,
             ...
             GetFullListDelaunay:=GetFullListDelaunay,
