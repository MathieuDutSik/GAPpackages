...
RandomPrimitiveElement:=function(eCase, PathInitial, IsSaving)
  local nbTry, DelCO, TheGramMat, RigidityKillingDelaunay, KillingAdjacency, iter, MaximalMemorySave, DimSpace, RetStr, TheReply;
  nbTry:=0;
  DimSpace:=Length(eCase.Basis);
  RigidityKillingDelaunay:=function(EXT_Delaunay, eInv)
    local pos, ListEqua;
    ListEqua:=ListEqualitiesDelaunay(EXT_Delaunay, eCase.Basis);
    pos:=First(ListEqua, x->x<>ListWithIdenticalEntries(DimSpace, 0));
    if pos<>fail then
      RetStr:=Concatenation("Kill with a |EXT|=", String(Length(EXT_Delaunay)));
      Print("RetStr=", RetStr, "\n");
      return RetStr;
    fi;
    return false;
  ...
  while(true)
  do
    Print("Before RandomPositiveElementFixedSymmetry\n");
    TheGramMat:=RandomPositiveElementFixedSymmetry(eCase);
    for iter in [1..12]
    do
      Print("Before DelaunayDescriptionStandardKernelInvFamily\n");
      TheReply:=DelaunayDescriptionStandardKernelInvFamily(TheGramMat, eCase.SymmGrpPtWs, eCase.CharacteristicFamilyVector, PathInitial, IsSaving, RigidityKillingDelaunay, KillingAdjacency, MaximalMemorySave);
      if TheReply.success then
        return rec(GramMat:=TheGramMat, DelCO:=TheReply.DelCO);
      fi;
      Print("error=", TheReply.error, "\n");
...
Periodic_RandomPrimitiveElement:=function(eCase, PathInitial, IsSaving)
  local nbTry, DelCO, TheGramMat, RigidityKillingDelaunay, KillingAdjacency, iter, ListGens, MatrixGRP, DimSpace, RetStr, TheReply;
  nbTry:=0;
  DimSpace:=Length(eCase.Basis);
  RigidityKillingDelaunay:=function(EXT_Delaunay, eInv)
    local pos, ListEqua;
  ...
  do
    ...
    for iter in [1..12]
    do
      TheReply:=Periodic_DelaunayDescriptionStandardKernel(TheGramMat, MatrixGRP, eCase.ListCosets, PathInitial, IsSaving, RigidityKillingDelaunay, KillingAdjacency);
      if TheReply.success then
        return rec(GramMat:=TheGramMat, DelCO:=TheReply.DelCO);
      fi;
      Print("error=", TheReply.error, "\n");
...
RandomPrimitiveElementNeighborhood:=function(eCase, PathInitial, IsSaving)
  local nbTry, DelCO, TheGramMat, KillingDelaunay, KillingAdjacency, W, Coeff, DimSpace, TheReply;
  nbTry:=0;
  DimSpace:=Length(eCase.Basis);
  W:=BasisExpansion(eCase.Basis, eCase.SuperMat);
  KillingDelaunay:=function(EXT_Delaunay, eInv)
  ...
  do
    ...
    while(true)
    do
      TheReply:=DelaunayDescriptionStandardKernel(TheGramMat, eCase.SymmGrpPtWs, PathInitial, IsSaving, KillingDelaunay, KillingAdjacency);
      if TheReply.success then
...
FindRepartitionningInfoNextGeneration:=function(eIdx, ListOrbitDelaunay, ListInformationsOneFlipping, InteriorElement, DataPolyhedralTiling)
  ...
  FuncInsertCenter:=function(TheRec)
    ...
    LVert:=ListOrbitDelaunay[TheRec.iDelaunay].EXT * TheRec.eBigMat;
    for eVert in LVert
    do
      FuncInsertVertex(eVert);
    od;
  ...
  FuncInsertCenter(rec(iDelaunay:=eIdx, eBigMat:=IdentityMat(n+1)));
  while(true)
  do
    IsFinished:=true;
...
FlippingLtype:=function(ListOrbitDelaunay, InteriorElement, ListInformationsOneFlipping, DataPolyhedralTiling)
  local Gra, n, ListMatched, eCase, eSet, ListConn, ListGroupMelt, ListGroupUnMelt, RepartInfo, INTS, eConn, ListInformationCenter, ListVertices, SYMGRP, LORB, ListInfo, NewListOrbitDelaunay, eRec, iDelaunay, iInfo, iOrb, i, DataTheEquivariantLtypeDomain, ListAdj, TheDATA, TheHistory, eAdj, TheOrb, TheOrbFacet, iDelaunayOld, Pos, iter, eMat2, eTrans2, BigMat2, AdjacentDelaunayCenterOld, iFacet, eFacet, EXT2, TheMat, g, TheFoundAdj, fAdj, Linc, eExt, iInc, iOrbFound, test, jFacet, WSR, WSR2, TheMat2, TheMat3, ImageEXT, LLinc, TheFoundAdj2, TheFoundAdj3, LincEXT, iV, eMat1, ImageEXTbarrel, kFacet, iDelaunayOrigin, jDelaunayOrigin, LLinc2, gAdj, hAdj, eMat3, EXT1, EXT3, EXT4, EXT5, EXT6, EXT7, LLinc3, iConn, jInfo, iFacet2, tAdj, LLinc4, BigMat1, TheFoundAdj4, TheMat4, TheFoundAdj1, TheMat1, jDelaunayOld, Pos2, ListCenterLow, ListCenterUpp, LORB2, ListCenterLow2, ListCenterUpp2;
  n:=Length(ListOrbitDelaunay[1].EXT[1])-1;
    ...
    if TheHistory[1]="old" then
      ...
      do
        ...
        if Pos<>fail then
          ...
          ImageEXT:=ListOrbitDelaunay[iDelaunayOld].EXT*eAdj.eBigMat;
      ...
      do
        ...
        if ListInfo[iInfo][jFacet].Position="barrel" then
          ...
          ImageEXT:=ListInfo[iInfo][kFacet].EXT * TheFoundAdj.eBigMat;
          ...
          ImageEXT:=ListOrbitDelaunay[jDelaunayOrigin].EXT*eMat3;
          ...
          EXT7:=ListInfo[jInfo][TheFoundAdj3.iOrb].EXT * BigMat1;
          ...
          eMat1:=eAdj.eBigMat;
          ImageEXT:=ListInfo[iInfo][jFacet].EXT*eMat1;
          ...
          if Pos<>fail then
            ...
            od;
            ImageEXT:=ListOrbitDelaunay[jDelaunayOld].EXT * TheFoundAdj1.eBigMat;
...
GroupAverage:=function(GRP, GramMat)
    local TheDim, ListGen, ListEqua, eGen, DiffMat, NSP, TheOrth, TotalBasis, ProjMatDiag, i, ProjMat;
    ...
    if Length(NSP)=0 then
        TheOrth:=IdentityMat(TheDim);
    else
        TheOrth:=NullspaceMat(GramMat * NSP);
    fi;
    TotalBasis:=Concatenation(NSP, TheOrth);
    #
    ProjMatDiag:=NullMat(TheDim, TheDim);
    for i in [1..Length(NSP)]
    do
        ProjMatDiag[i][i]:=1;
    od;
    ProjMat:=Inverse(TotalBasis) * ProjMatDiag * TotalBasis;
    return ProjMat;
end;
...
ComputeQuantizationPolynomialFraction:=function(OneLtype, eCase)
    local n, DimSpace, FAC, EXT, eCent, GramMatInterior, i;
    n:=Length(eCase.SuperMat);
    DimSpace:=Length(eCase.Basis);
    FAC:=WriteFaceInequalities(OneLtype, eCase);
    EXT:=DualDescriptionStandard(FAC);
    eCent:=Sum(EXT);
    GramMatInterior:=NullMat(n,n);
    for i in [1..DimSpace]
    do
        GramMatInterior:=GramMatInterior + eCent[i] * eCase.Basis[i];
    od;
    #
...
#    Print("Just before insert\n");
  ...
  od;
  Print("EnumerationProcedureLtypeCore finished nbOrbit=", nbOrbit, "\n");
end;
...
#    ThePrefix:=Concatenation(PathSave, "CoopPacoop/");
  ...
  FuncGetAllExtremeRays:=function()
    ...
    do
      ...
      od;
      Add(ListListExtremeRay, rec(ListGramEXT:=ListExtremeRay, EXT:=EXT2));
    od;
...
EnumerationProcedureLtype:=function(eCase)
  ...
  Print("Before EnumerationProcedureLtypeStandard\n");
