...
PrintListAdjCppPolyhedral:=function(eFile, ListAdjacency)
  local output, nbV, LAdj, eVal, iV;
  output:=OutputTextFile(eFile, true);
  nbV:=Length(ListAdjacency);
  AppendTo(output, nbV, "\n");
  for iV in [1..nbV]
    do
      LAdj:=ListAdjacency[iV];
...
PrintGraphCppPolyhedral_TrivialColor:=function(eFile, GRA)
  local output, nbV, LAdj, eVal, iV;
  output:=OutputTextFile(eFile, true);
  nbV:=GRA.order;
  AppendTo(output, nbV, "\n");
...
GRAPH_EnumerateCycles:=function(TheGRA, GRPvert, RecOpt)
  local nbVert, O, ListOrb, iK, NewListOrb, FuncInsert, eOrb, eVert, fOrb, ePerm1, ePerm2, Canonicalize, GetTheCompleteOrbit, GetTotal, FinalListOrb, CycleDistance, IsIsometricCycle, TheDiff, FuncInsertFinal, ListListAdj, ListEdge, LAdj, IndexMatrix, GetEdgeSet, fVert, nbEdge, eEdge, iEdge, pos, ListPermEdge, eGen, eList, eEdgeImg, IsExtensibleIrreducible, IsClosed, GRPedge, IsFinalIrreducible, IsCorrectFinal, IsCorrectEnum, TestEquivalenceFullCycle, IsIsometricCycle_General, IsIsometricCycleFixedLength, IsExtendibleIrreducible, testClosed, UseMinimumOrbit;
  nbVert:=TheGRA.order;
  ListListAdj:=[];
  ListEdge:=[];
  for eVert in [1..nbVert]
  ...
  Canonicalize:=function(eCycle)
    local DihedGrp;
    DihedGrp:=MyDihedralGroup(Length(eCycle));
