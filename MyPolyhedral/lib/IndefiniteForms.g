SAGE_GetLocalSymbols:=function(M)
  local FileInput, FileOutput, output, TheCommand, RecInv;
  FileInput:=Filename(POLYHEDRAL_tmpdir, "LocalSymbols.sage");
  FileOutput:=Filename(POLYHEDRAL_tmpdir, "LocalSymbols.out");
  output:=OutputTextFile(FileInput, true);
  AppendTo(output, "M = ");
  SAGE_PrintMatrix(output, M);
  AppendTo(output, "\n");
  AppendTo(output, "G = Genus(M)\n");
  AppendTo(output, "print(\"return rec(strinv:=\\\"\" + str(G.local_symbols()) + \"\\\");\")");
  CloseStream(output);
#  Print("FileInput  = ", FileInput, "\n");
#  Print("FileOutput = ", FileOutput, "\n");
  TheCommand:=Concatenation("sage ", FileInput, " > ", FileOutput);
  Exec(TheCommand);
  RecInv:=ReadAsFunction(FileOutput)();
...
# is only the flag preserving one
...
INDEF_FORM_Machinery_AllFct:=function()
    ...
    INDEF_FORM_TestEquivalence_Kernel:=function(Qmat1, Qmat2)
        ...
        if Block1.h = 1 then
            ...
            SaveDebugInfo("LORENTZ_TestIsomorphism", rec(mat1:=Block1.mat, mat2:=Block2.mat, test:=test));
            if test=false then
                return fail;
            fi;
            return test;
    ...
    INDEF_FORM_AutomorphismGroup_Kernel:=function(Qmat)
        local eBlock, ListGenerators, ApproxModel, eRec, X, v1, v2, test, eGen, f_insert, GRPlor;
        ...
        if eBlock.h = 1 then
            GRPlor:=LORENTZ_OrthogonalGroup(eBlock.mat);
            SaveDebugInfo("LORENTZ_OrthogonalGroup", rec(mat:=eBlock.mat, GRPlor:=GRPlor));
            return GRPlor;
        fi;
        ListGenerators:=[];
...
GAP_INDEF_FORM_GetOrbitRepresentative:=function(Qmat, X)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_GetOrbitRepresentative(Qmat,X);
end;

GAP_INDEF_FORM_StabilizerVector:=function(Qmat, v)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_StabilizerVector(Qmat, v);
end;

GAP_INDEF_FORM_EquivalenceVector:=function(Qmat1, Qmat2, v1, v2)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_EquivalenceVector(Qmat1, Qmat2, v1, v2);
end;

GAP_INDEF_FORM_AutomorphismGroup:=function(Qmat)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_AutomorphismGroup(Qmat);
end;

GAP_INDEF_FORM_TestEquivalence:=function(Qmat1, Qmat2)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_TestEquivalence(Qmat1, Qmat2);
end;

# isotropic planes

GAP_INDEF_FORM_Invariant_IsotropicKplane:=function(Qmat, Plane)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_Invariant_IsotropicKplane(Qmat, Plane);
end;

GAP_INDEF_FORM_Equivalence_IsotropicKplane:=function(Qmat1, Qmat2, Plane1, Plane2)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_Equivalence_IsotropicKplane(Qmat1, Qmat2, Plane1, Plane2);
end;

GAP_INDEF_FORM_Stabilizer_IsotropicKplane:=function(Qmat, Plane)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_Stabilizer_IsotropicKplane(Qmat, Plane);
end;

GAP_INDEF_FORM_RightCosets_IsotropicKplane:=function(Qmat, Plane)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_RightCosets_IsotropicKplane(Qmat, Plane);
end;

GAP_INDEF_FORM_GetOrbit_IsotropicKplane:=function(Qmat, k)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_GetOrbit_IsotropicKplane(Qmat, k);
end;

#

GAP_INDEF_FORM_Invariant_IsotropicKflag:=function(Qmat, Plane)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_Invariant_IsotropicKflag(Qmat, Plane);
end;

GAP_INDEF_FORM_Equivalence_IsotropicKflag:=function(Qmat1, Qmat2, Plane1, Plane2)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_Equivalence_IsotropicKflag(Qmat1, Qmat2, Plane1, Plane2);
end;

GAP_INDEF_FORM_Stabilizer_IsotropicKflag:=function(Qmat, Plane)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_Stabilizer_IsotropicKflag(Qmat, Plane);
end;

GAP_INDEF_FORM_RightCosets_IsotropicKflag:=function(Qmat, Plane)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_RightCosets_IsotropicKflag(Qmat, Plane);
end;

GAP_INDEF_FORM_GetOrbit_IsotropicKflag:=function(Qmat, k)
    return INDEF_FORM_Machinery_AllFct().INDEF_FORM_GetOrbit_IsotropicKflag(Qmat, k);
end;


INDEF_FORM_InvariantAutomorphism:=function(Qmat, GRP)
    local n, ListInv, p, Fset, Fmap, ListPerm, O, eInv;
    n:=Length(Qmat);
    ListInv:=[];
    for p in [2,3,4]
    do
        Fset:=Set(BuildSet(n, [0..p-1]));
        Print("p=", p, " |Fset|=", Length(Fset), "\n");
        Fmap:=function(eGen)
            return SortingPerm(Fset * eGen);
        end;
        ListPerm:=List(GeneratorsOfGroup(GRP), Fmap);
        O:=Orbits(Group(ListPerm), [1..Length(Fset)], OnPoints);
        eInv:=Collected(List(O, Length));
        Add(ListInv, eInv);
    od;
    return ListInv;
end;
...
INDEF_FORM_TestAutomorphismIsomorphism:=function(Qmat_A)
    local n, TheP, GRP_A, eInvA, n_iter, iter, Qmat_B, GRP_B, eInvB, test, CheckAutom, CheckIsom;
    n:=Length(Qmat_A);
    TheP:=IdentityMat(n);
    CheckAutom:=false;
    CheckIsom:=true;
