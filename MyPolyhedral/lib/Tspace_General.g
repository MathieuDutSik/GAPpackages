...
#    Print("Before ArithmeticAutomorphismGroup\n");
    GRP:=ArithmeticAutomorphismGroup_InputBasis([TheMat], eCase.CharacteristicFamilyVector);
#    Print("After ArithmeticAutomorphismGroup\n");
...
WriteLinSpace:=function(output, eCase)
    local ListSubspace;
    CPP_WriteMatrix(output, eCase.SuperMat);
    WriteListMatrix(output, eCase.Basis);
    WriteListMatrix(output, eCase.ListComm);
    ListSubspace:=[];
    if IsBound(eCase.ListSubspace) then
        ListSubspace:=eCase.ListSubspace;
    fi;
    WriteListMatrix(output, ListSubspace);
    WriteListMatrix(output, GeneratorsOfGroup(eCase.SymmGrpPtWs));
end;

WriteLinSpaceFile:=function(eFile, eCase)
    local output;
    RemoveFileIfExist(eFile);
    output:=OutputTextFile(eFile, true);
    WriteLinSpace(output, eCase);
    CloseStream(output);
end;
