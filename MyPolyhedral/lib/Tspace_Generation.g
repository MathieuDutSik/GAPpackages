...
TspaceFromOrthogonal:=function(GramMat, V)
    local n, Bmat, V2, TheBasis, IsBravaisSpace, SuperMat, SVR, SymmGrpPtWs, ListCosets, eSubspace, ListSubspace;
    n:=Length(GramMat);
    V2:=[V];
    Bmat:=GramMat*TransposedMat(V2)*V2*GramMat;
    TheBasis:=[GramMat, Bmat];
    IsBravaisSpace:=false;
    SuperMat:=GramMat;
    SVR:=ShortestVectorDutourVersion(GramMat);
    SymmGrpPtWs:=ArithmeticAutomorphismGroup(TheBasis);
    eSubspace:=[V];
    ListSubspace:=[eSubspace];
    return rec(Basis:=TheBasis, IsBravaisSpace:=IsBravaisSpace, ListComm:=[], ListSubspace:=ListSubspace, SymmGrpPtWs:=SymmGrpPtWs, SuperMat:=SuperMat, CharacteristicFamilyVector:=SVR);
end;

TspaceFromInvariantVector:=function(GramMat, V)
    local SymmGrpPtWs, ListMatrGen, eO, ListPermGen, PermGRP, phi, pos, eStabPerm, eStabMatr, TheBasis, IsBravaisSpace;
    SymmGrpPtWs:=ArithmeticAutomorphismGroup([GramMat]);
    ListMatrGen:=GeneratorsOfGroup(SymmGrpPtWs);
    eO:=Set(Orbit(SymmGrpPtWs, V, OnPoints));
    ListPermGen:=List(ListMatrGen, eGen->SortingPerm(eO * eGen));
    PermGRP:=Group(ListPermGen);
    phi:=GroupHomomorphismByImagesNC(PermGRP, SymmGrpPtWs, ListPermGen, ListMatrGen);
    pos:=Position(eO, V);
    eStabPerm:=Stabilizer(PermGRP, pos, OnPoints);
    eStabMatr:=Image(phi, eStabPerm);
    TheBasis:=InvariantFormDutourVersion(GeneratorsOfGroup(eStabMatr));
    IsBravaisSpace:=true;
    return rec(Basis:=TheBasis, IsBravaisSpace:=IsBravaisSpace, ListComm:=[], ListSubspace:=[], SymmGrpPtWs:=eStabMatr, SuperMat:=GramMat);
end;

TspaceLamination:=function(GramMat, SingleDelaunay)
  local TheBasis, n, H, K, i, j, ListGen, ListGenDelaunay, eMat, TheExpl, W, NewMat, SPA, eCentRed, SuperMat, v, Ord, IsBravaisSpace, SVR, SymmGrpPtWs, GRP;
  n:=Length(GramMat);
  H:=NullMat(n+1, n+1);
  for i in [1..n]
  do
  ...
  od;
  Ord:=Order(SingleDelaunay.TheStab.PermutationStabilizer);
  ListGen:=[];
...
#  SVR:=__ExtractInvariantFaithfulFamilyShortVector(SuperMat, GRP);
  SVR:=__ExtractInvariantZBasisShortVectorNoGroup(SuperMat);
  Print("|SVR|=", Length(SVR), "\n");
  SymmGrpPtWs:=ArithmeticAutomorphismGroup(Concatenation([SuperMat], TheBasis));
  Print("We have SymmGrpPtWs\n");
  return rec(Basis:=TheBasis, SymmGrpPtWs:=SymmGrpPtWs, Order:=Ord, IsBravaisSpace:=IsBravaisSpace, ListGen:=ListGen, SuperMat:=SuperMat, CharacteristicFamilyVector:=SVR);
end;
...
GetSpaceRealQuadratic:=function(n, eSum, eProd)
  ...
  return rec(Basis:=TheBasis,
             ...
             SymmGrpPtWs:=Group([IdentityMat(2*n)]),
...
GetRationalQuaternionAlgebra:=function(n, QuartAlg)
  ...
  return rec(Basis:=TheBasis,
             ...
             IsInSL:=IsInSL, IsInGL:=IsInGL,
             TrivialFilter:=TrivialFilter,
             SymmGrpPtWs:=Group([IdentityMat(4*n)]),
             IsAdmissible:=IsPositiveDefiniteSymmetricMatrix,
             ShortestFunction:=ShortestVectorDutourVersion);
end;
...
GetTotallyRealFieldExpression:=function(n, ListS)
  ...
  return rec(Basis:=TheBasis,
             ...
             IsInGL:=IsInGL,
             TrivialFilter:=TrivialFilter,
             SymmGrpPtWs:=Group([IdentityMat(TheDeg*n)]),
             IsAdmissible:=IsPositiveDefiniteSymmetricMatrix,
             ShortestFunction:=ShortestVectorDutourVersion);
end;
