...
FileGetQNdouble:=Filename(DirectoriesPackagePrograms("MyPolyhedral"),"GetQNdouble");
...
GetSuperlattices:=function(GramMat, GRP, TheMod)
  ...
  do
    ...
    if eVect*eVect>0 then
      NewBasis:=SubspaceCompletionInt([eVect], Length(eVect));
...
GetPointGroupFromCrystallographicGroup:=function(GRP)
  local ListGensRed, eGen, eXp;
  ListGensRed:=[];
  for eGen in GeneratorsOfGroup(GRP)
  do
    eXp:=FuncExplodeBigMatrix(eGen);
    Add(ListGensRed, eXp.MatrixTransformation);
  od;
  return Group(ListGensRed);
end;
...
QN_GetQNdouble:=function(Nval, eVal)
  local ePair, FileOut, TheReply;
  ePair:=QN_GetExpression(Nval, eVal);
  FileOut:=Filename(POLYHEDRAL_tmpdir, "GetQN.out");
  RemoveFileIfExist(FileOut);
  Exec(FileGetQNdouble, " ", String(Nval), " ", String(ePair[1]), " ", String(ePair[2]), " > ", FileOut);
  TheReply:=ReadAsFunction(FileOut)();
  RemoveFile(FileOut);
  return TheReply;
end;

Q2_CriticalInformation:=function(GramMat)
  local n, GramMat1, GramMat2, i, j, ePair, Nord, eApprox, GramMatRat, TheRemainder, TheDiag, MiniN, MaxiN, eVal, InvTrans, GramMatTrans, TheTransform, res;
  n:=Length(GramMat);
  ...
  do
    for j in [1..n]
    do
      ePair:=QN_GetExpression(2, GramMat[i][j]);
  ...
  while(true)
  do
    eApprox:=NorderApproximationSqrt2(Nord);
    GramMatRat:=GramMat1+GramMat2*eApprox;
    if IsPositiveDefiniteSymmetricMatrix(GramMatRat)=true then
      res:=LLLReducedGramMat(GramMatRat);
      TheRemainder:=res.remainder;
      TheTransform:=res.transformation;
      InvTrans:=Inverse(TheTransform);
      if InvTrans*TheRemainder*TransposedMat(InvTrans)<>GramMatRat then
        Error("Error in LLL computation");
      fi;
      GramMatTrans:=TheTransform*GramMat*TransposedMat(TheTransform);
      TheDiag:=List([1..n], x->GramMatTrans[x][x]);
      MiniN:=TheDiag[1];
      MaxiN:=TheDiag[1];
      for i in [2..n]
      do
        eVal:=TheDiag[i];
        if QN_IsPositive(2, eVal-MaxiN)=true then
          MaxiN:=eVal;
        fi;
        if QN_IsPositive(2, MiniN-eVal)=true then
          MiniN:=eVal;
        fi;
      od;
      return rec(ShortestVal:=MiniN, CharVal:=MaxiN);
    fi;
    Nord:=Nord+1;
  od;
end;
...
Q2_ShortestVectors:=function(GramMat, MaxNorm)
  local n, GramMatInv, GramMat1, GramMat2, i, j, ePair, ePairApprox, eApprox1, eApprox2, GetMajorizingConstant, TheTol, vectors, norms, eVect, eNorm, SHV, MaxNormUpper, MajorDist, MaxValueCoeff, MaxNormRat, eCoeffInv, GetApproximant, GetUpperRational, MaxNormSecondTerm, GramMatApprox, eCoeffInvRat;
  n:=Length(GramMat);
  ...
  do
    ...
    od;
  od;
  GetApproximant:=function(TheTol)
    local Nord, eApprox1, eApprox2, eMajorDiff;
    Nord:=1;
    while(true)
    do
      eApprox1:=NorderApproximationSqrt2(Nord);
      eApprox2:=NorderApproximationSqrt2(Nord+1);
      eMajorDiff:=AbsInt(eApprox1-eApprox2);
      if eMajorDiff<TheTol then
        return [eApprox1, eApprox2];
      fi;
      Nord:=Nord+1;
    od;
  end;
  # return the a such that eMat <= a Id
  # in the sense of symmetric matrices
  # eMat is assumed to be positive definite
  # which is not essential
  GetMajorizingConstant:=function(eMat)
    local p, ListDiagMajor, i, j, eVal, absVal, TheMaxVal;
    p:=Length(eMat);
    ListDiagMajor:=ListWithIdenticalEntries(p,0);
    for i in [1..p]
    do
      ListDiagMajor[i]:=eMat[i][i];
    od;
    for i in [1..p-1]
    do
      for j in [i+1..p]
      do
        eVal:=eMat[i][j];
        if QN_IsPositive(2, eVal)=true then
          absVal:=eVal;
        else
          absVal:=-eVal;
        fi;
        ListDiagMajor[i]:=ListDiagMajor[i]+absVal;
        ListDiagMajor[j]:=ListDiagMajor[j]+absVal;
      od;
    od;
    TheMaxVal:=0;
    for i in [1..p]
    do
      if QN_IsPositive(2, ListDiagMajor[i]-TheMaxVal) then
        TheMaxVal:=ListDiagMajor[i];
      fi;
    od;
    return TheMaxVal;
  end;
  TheTol:=10^(-10);
  ePairApprox:=GetApproximant(TheTol);
  eApprox1:=ePairApprox[1];
  eApprox2:=ePairApprox[2];
#  Print("eApprox1=", eApprox1, " eApprox2=", eApprox2, "\n");
  GetUpperRational:=function(eVal)
    local ePairVal, eValApprox1, eValApprox2;
    ePairVal:=QN_GetExpression(2, eVal);
    eValApprox1:=ePairVal[1]+eApprox1*ePairVal[2];
    eValApprox2:=ePairVal[1]+eApprox2*ePairVal[2];
    return Maximum(eValApprox1, eValApprox2);
  end;
  eCoeffInv:=GetMajorizingConstant(GramMatInv);
#  Print("We have eCoeffInv\n");
  eCoeffInvRat:=GetUpperRational(eCoeffInv);
#  Print("We have eCoeffInvRat\n");
  MaxNormRat:=GetUpperRational(MaxNorm);
#  Print("We have MaxNormRat\n");
#  Print("MaxNormRat*eCoeffInv=", MaxNormRat*eCoeffInv, "\n");
  MaxValueCoeff:=GetLowestPower(2, MaxNormRat*eCoeffInvRat);
#  Print("MaxValueCoeff=", MaxValueCoeff, "\n");
#  Print("We have MaxValueCoeff\n");
  MajorDist:=AbsInt(eApprox1-eApprox2);
#  Print("We have MajorDist\n");
  MaxNormSecondTerm:=MajorDist*n*(MaxValueCoeff^2)*GetMajorizingConstant(GramMat2);
#  Print("We have MaxNormSecondTerm\n");
  GramMatApprox:=GramMat1+eApprox2*GramMat2;
#  Print("We have GramMatApprox\n");
#  PrintArray(GramMatApprox);
  MaxNormUpper:=MaxNormRat+MaxNormSecondTerm;
  SHV:=ShortVectorDutourVersion(GramMatApprox, MaxNormUpper);
  vectors:=[];
  norms:=[];
  for eVect in SHV
  do
    eNorm:=eVect*GramMat*eVect;
    if QN_IsNonNegative(2, MaxNorm-eNorm)=true then
      Add(vectors, eVect);
      Add(norms, eNorm);
    fi;
  od;
