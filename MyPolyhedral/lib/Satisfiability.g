...
PrintForSpectralLowerBound:=function(FileName, GRA)
  local output, iVert, LAdj, eAdj;
  RemoveFileIfExist(FileName);
  output:=OutputTextFile(FileName, true);
  AppendTo(output, GRA.order, "\n");
  for iVert in [1..GRA.order]
  do
    LAdj:=Adjacency(GRA, iVert);
    AppendTo(output, Length(LAdj));
    for eAdj in LAdj
    do
      AppendTo(output, " ", eAdj-1);
    od;
    AppendTo(output, "\n");
  od;
  CloseStream(output);
...
GetVectorColoring:=function(ThePartition)
    local nbVert, nbColor, VectorColoring, i;
    nbVert:=Maximum(List(ThePartition, Maximum));
    nbColor:=Length(ThePartition);
    VectorColoring:=ListWithIdenticalEntries(nbVert, -1);
    for i in [1..nbColor]
    do
        VectorColoring{ThePartition[i]} := ListWithIdenticalEntries(Length(ThePartition[i]), i);
    od;
    return VectorColoring;
end;


GetColoring:=function(SolCNF, nbVert, nbColor)
  local MatrixIndex, pos, i, eC, ThePartition, NewPartition;
  ...
  do
    ...
    od;
  od;
  NewPartition:=Set(List(ThePartition, Set));
  return rec(ThePartition:=NewPartition, VectorColoring:=GetVectorColoring(NewPartition));
end;
...
GetRandomColoring:=function(GRA, nbColor, n_iter)
    local SetColoring, nbVert, GRP, iter, ePerm, ePermInv, GRAmap, i, iMap, j, jMap, ListCNF, SolCNF, TheRec, ThePartition, ePart, ePartMap, NewPartition, NewRec;
    SetColoring:=[];
    nbVert:=GRA.order;
    GRP:=SymmetricGroup(nbVert);
    for iter in [1..n_iter]
    do
        ePerm:=Random(GRP);
        ePermInv:=Inverse(ePerm);
        GRAmap:=NullGraph(Group(()), nbVert);
        for i in [1..nbVert]
        do
            iMap:=OnPoints(i, ePerm);
            for j in Adjacency(GRA, i)
            do
                jMap:=OnPoints(j, ePerm);
                AddEdgeOrbit(GRAmap, [iMap, jMap]);
            od;
        od;
        ListCNF:=GetSatisfiabilityForColoring(GRAmap, nbColor);
        SolCNF:=SolveCNF(ListCNF);
        TheRec:=GetColoring(SolCNF, nbVert, nbColor);
        if TheRec = false then
            return false;
        fi;
        ThePartition:=[];
        for ePart in TheRec.ThePartition
        do
            ePartMap:=Set(List(ePart, x->OnPoints(x, ePermInv)));
            Add(ThePartition, ePartMap);
        od;
        NewPartition:=Set(ThePartition);
        NewRec:=rec(ThePartition:=NewPartition, VectorColoring:=GetVectorColoring(NewPartition));
        AddSet(SetColoring, NewRec);
    od;
    return SetColoring;
end;
