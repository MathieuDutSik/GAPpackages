...
GetSparseMatFromGAPmatrix:=function(eMat, orientation)
    local nbLine, nbCol, ListEntries, iLine, ListCol, ListVal, iCol, eVal, eEnt;
    if orientation="direct" then
        nbLine:=Length(eMat);
        nbCol:=Length(eMat[1]);
        ListEntries:=[];
        for iLine in [1..nbLine]
    ...
    if orientation="transpose" then
        nbLine:=Length(eMat[1]);
        nbCol:=Length(eMat);
        ListEntries:=[];
        for iLine in [1..nbLine]
        do
    ...
    Error("Allowed options are direct and transpose");
end;


Image_AX:=function(SpMat, eVect)
  local nbLine, nbCol, eVectRet, iLine, eSum, eEnt, len, i, iCol, eVal;
  nbLine:=SpMat.nbLine;
  nbCol:=SpMat.nbCol;
  if nbCol<>Length(eVect) then
      Print("nbCol=", nbCol, " |eVect|=", Length(eVect), "\n");
      Error("Incoherent input in Image_AX");
  fi;
  eVectRet:=[];
  for iLine in [1..nbLine]
  do
...
Image_trAX:=function(SpMat, eVect)
  local nbLine, nbCol, eVectRet, iLine, eEnt, len, i, iCol, eVal;
  nbLine:=SpMat.nbLine;
  nbCol:=SpMat.nbCol;
  if nbLine<>Length(eVect) then
    Print("nbLine=", nbLine, " |eVect|=", Length(eVect), "\n");
    Error("Incoherent input in Image_trAX");
  fi;
  eVectRet:=ListWithIdenticalEntries(nbCol, 0);
  for iLine in [1..nbLine]
  do
    eEnt:=SpMat.ListEntries[iLine];
    len:=Length(eEnt.ListCol);
    ...
    do
      iCol:=eEnt.ListCol[i];
      eVal:=eEnt.ListVal[i];
      eVectRet[iCol]:=eVectRet[iCol] + eVal * eVect[iLine];
    od;
  od;
  return eVectRet;
end;
...
#
# Returns the NullspaceMat(TransposedMat(eMat));
# But it has little chance of success as it works
# only for finite fields.
#
LINBOX_NullspaceB:=function(RecSparseMat)
  local FileMat, FileOut, FileOutB, FileErr, FileErrB, FileGap, TheCommand, TheResult, nbLine, nbCol, IsFamilyInKernel, eExpo, ePrime, DoClean;
  nbLine:=RecSparseMat.nbLine;
  nbCol:=RecSparseMat.nbCol;
  FileMat:=Filename(POLYHEDRAL_tmpdir,"mat.txt");
  FileOut:=Filename(POLYHEDRAL_tmpdir,"out.txt");
  FileOutB:=Filename(POLYHEDRAL_tmpdir,"outb.txt");
  FileErr:=Filename(POLYHEDRAL_tmpdir,"err.txt");
  FileErrB:=Filename(POLYHEDRAL_tmpdir,"errb.txt");
  FileGap:=Filename(POLYHEDRAL_tmpdir,"out.gap");
  RemoveFileIfExist(FileMat);
  RemoveFileIfExist(FileOut);
  RemoveFileIfExist(FileErr);
  RemoveFileIfExist(FileErrB);
  RemoveFileIfExist(FileGap);
  LINBOX_SparsePrint(FileMat, RecSparseMat);
  #
  IsFamilyInKernel:=function(NSP)
      local eNSP, eImg;
      Print("|NSP|=", Length(NSP), "\n");
      for eNSP in NSP
      do
          eImg:=Image_AX(RecSparseMat, eNSP);
          Print("eImg=", eImg, "\n");
  ...
  while(true)
  do
    ePrime:=First([eExpo..2*eExpo], IsPrime);
    #
    TheCommand:=Concatenation(FileCompLinboxNullspace, " ", FileMat, " ", String(ePrime), " > ", FileOut, " 2> ", FileErr);
    Print("TheCommand A=", TheCommand, "\n");
    Exec(TheCommand);
    #
    TheCommand:=Concatenation(FileCompLinboxNullspaceRead, " ", FileErr, " ", FileGap, " > ", FileOutB, " 2> ", FileErrB);
    Print("TheCommand B=", TheCommand, "\n");
    Exec(TheCommand);
    #
    Print(NullMat(5));
    TheResult:=ReadAsFunction(FileGap)();
    #
    if IsFamilyInKernel(TheResult) then
        Print(NullMat(5));
        DoClean();
        RemoveFileIfExist(FileMat);
        return TheResult;
...
LINBOX_NullspaceC:=function(RecSparseMat)
  local FileMat, FileOut, FileOutB, FileErr, FileErrB, FileGap, TheCommand, TheResult, nbLine, nbCol, IsFamilyInKernel, eExpo, ePrime, DoClean;
  nbLine:=RecSparseMat.nbLine;
  nbCol:=RecSparseMat.nbCol;
  FileMat:=Filename(POLYHEDRAL_tmpdir,"mat.txt");
  ...
  #
  TheCommand:=Concatenation(FileCompLinboxNullspaceRat, " ", FileMat, " > ", FileOut, " 2> ", FileErr);
  Print("TheCommand A=", TheCommand, "\n");
  Exec(TheCommand);
  #
  TheCommand:=Concatenation(FileCompLinboxNullspaceRead, " ", FileErr, " ", FileGap, " > ", FileOutB, " 2> ", FileErrB);
  ...
  DoClean();
  #
  return List(TheResult, RemoveFraction);
end;


LINBOX_Nullspace_Direct:=function(eMat)
    local RecSparseMat;
    RecSparseMat:=GetSparseMatFromGAPmatrix(eMat, "transpose");
    return LINBOX_NullspaceC(RecSparseMat);
end;


LINBOX_Nullspace_Transpose:=function(eMat)
    local RecSparseMat;
    RecSparseMat:=GetSparseMatFromGAPmatrix(eMat, "direct");
    return LINBOX_NullspaceC(RecSparseMat);
end;
