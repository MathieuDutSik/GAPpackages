...
GetMainComponent:=function(EXT)
    local eRecDecomp, TotalSet, eEnt, EXTirr;
    eRecDecomp:=DirectSumDecompositionPolyhedralCone(EXT);
    TotalSet:=[];
    for eEnt in eRecDecomp.ListListIdx
    do
        if Length(eEnt)>1 then
            Append(TotalSet, eEnt);
        fi;
    od;
    EXTirr:=EXT{TotalSet};
    return EXTirr;
...
#    DataPolyhedral.DualDescriptionFunction:=__DualDescriptionLRS_Reduction;
    DataPolyhedral.DualDescriptionFunction:=__DualDescriptionCDD_Reduction;
