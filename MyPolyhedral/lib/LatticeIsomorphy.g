...
__ExtractInvariantZBasisShortVectorNoGroup_UsingCVP:=function(GramMat)
  ...
  while(true)
  do
    TheSub:=SubspaceCompletionInt(TheSpann, n);
...
GetRecSVR_FromInvFamily:=function(GramMat, PointGRP, InvariantFamily)
  local n, TheInvariantBase, WorkingAffineBasis, SVRfaithful;
  n:=Length(GramMat);
  Print("Before UseExistingForAffineBasis\n");
  WorkingAffineBasis:=UseExistingForAffineBasis(InvariantFamily);
  Print("Before Kernel_ExtractInvariantFaithfulFamilyShortVector\n");
  if Length(InvariantFamily) < 1000 then
    SVRfaithful:=InvariantFamily;
  else
    SVRfaithful:=Kernel_ExtractInvariantFaithfulFamilyShortVector(GramMat, PointGRP);
  fi;
  return rec(TheInvariantBase:=InvariantFamily,
             WorkingAffineBasis:=WorkingAffineBasis,
             SVRfaithful:=SVRfaithful);
end;
...
ArithmeticAutomorphismMatrixFamily_Nauty:=function(ListMat, SVR)
  ...
  if RankMat(SVR)<>Length(ListMat[1][1]) then
    Print("RankMat(SVR)=", RankMat(SVR), "\n");
    Print("dim=", Length(ListMat[1][1]), "\n");
...
GetReductionMatrix:=function(eGram)
  ...
  eCompl:=SubspaceCompletionInt(NSP, n);
