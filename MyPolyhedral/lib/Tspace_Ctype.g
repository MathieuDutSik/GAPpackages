...
CTYP_TestUnimodularFamily:=function(ListVect)
  local ListMatrix, ListMatVect, eVect, eMat, TheSuperMat, TheFilter, ListVectTot, GRPlin, ListMatGen, eGen, TheMat, SymmGrpPtWs, eCase, KillingDelaunay, KillingAdjacency, IsSaving, PathInitial, MatrixGRP, n, MaximalMemorySave, DelCO, TheCType, nbVect, IsCtype, iVect;
  ListMatrix:=[];
  ListMatVect:=[];
  n:=Length(ListVect[1]);
...
RetrievingNonRedundantFacet:=function(FAC, GRPfac)
  ...
  # Working with Clarkson algorithm
  ListPos:=CPP_RedundancyReductionClarkson(FAC);
  return ListPos;
  #
  nbFac:=Length(FAC);
...
CTYP_GetGramMatricesExtremeRays:=function(TheBasis, RecIneq, n)
  ...
  TheDim:=Length(TheBasis);
  ListIneqRed:=RecIneq.ListInequalities{ListPos};
...
CTYP_GetCanonicalEntryForLowerBound:=function(TheLtype)
    local nbSimp, n, ListVect, ListSumVol, CanonicalizeVector, eSimp, i, j, eDiff, CanVect, pos, EXTexpand, ListVolExpand, eVol, eRec;
    nbSimp:=Length(TheLtype);
    n:=Length(TheLtype[1].EXT[1])-1;
    ListVect:=[];
    ListSumVol:=[];
    CanonicalizeVector:=function(eVect)
        local idx;
        for idx in [1..n]
        do
            if eVect[idx] > 0 then
    ...
    for eSimp in TheLtype
    do
        eVol:=AbsInt(DeterminantMat(eSimp.EXT));
        for i in [1..n+1]
        do
            for j in [i+1..n+1]
            do
#                Print("i=", i, " j=", j, "\n");
                eDiff:=eSimp.EXT[i]{[2..n+1]} - eSimp.EXT[j]{[2..n+1]};
                CanVect:=CanonicalizeVector(eDiff);
                pos:=Position(ListVect, CanVect);
                if pos=fail then
                    Add(ListVect, CanVect);
                    Add(ListSumVol, eVol);
                else
                    ListSumVol[pos]:=ListSumVol[pos] + eVol;
                fi;
            od;
        od;
    od;
    EXTexpand:=Concatenation(ListVect, -ListVect);
    ListVolExpand:=Concatenation(ListSumVol, ListSumVol);
    eRec:=LinPolytopeIntegral_CanonicalForm(EXTexpand);
    Print(NullMat(5));
    return [eRec.EXT, ListVolExpand{eRec.CanonicalList}];
end;
...
CTYP_SpanCone:=function(TheCtype, TheBasis, n)
  local ListParity, ListResidue, ListCharVect, RecIneq, ListGram, ListVect, eGram, eVect, RecEXT;
  ...
  RecEXT:=CTYP_GetGramMatricesExtremeRays(TheBasis, RecIneq, n);
  #
  ListVect:=[];
  for eGram in RecEXT.ListGram
  do
    eVect:=List(ListCharVect, x->x*eGram*x);
    Add(ListVect, RemoveFraction(eVect));
  od;
  return rec(ListVect:=ListVect, ListGram:=RecEXT.ListGram, EXT:=RecEXT.EXT);
end;
...
CTYP_ComputingCtypeExtremeRank:=function(EXT, GramMat)
    local n, nbVert, TheBasis, EXTred, EXT_expand, FAC_expand, ListPoint, ListPointFilt, ListEqua, f_norm, ePoint, ePointRed, ListNorm, MinNorm, ListIdx, ListPointSel, ListPointSelB, ePt1, iPt, ePt2, eEqua, eMat, NSP, CtypeSpace, eNSP, i, TheMat;
    n:=Length(EXT[1]) - 1;
    nbVert:=Length(EXT);
    TheBasis:=CTYP_GetBasis(n);
    EXTred:=List(EXT, x->x{[2..n+1]});
    EXT_expand:=List(EXT, x->Concatenation([1], 2*x{[2..n+1]}));
    FAC_expand:=DualDescription(EXT_expand);
    ListPoint:=GetIntegralPointsIteration_Kernel(FAC_expand, EXT_expand);
    ListPointFilt:=Filtered(ListPoint, x->IsIntegralVector(x / 2)=false);
    ListEqua:=[];
    f_norm:=function(x)
        return x * GramMat * x;
    end;
    Print("  |ListPointFilt|=", Length(ListPointFilt), "\n");
    for ePoint in ListPointFilt
    do
        ePointRed:=ePoint / 2;
        ListNorm:=List(EXTred, x->f_norm(x - ePointRed));
        MinNorm:=Minimum(ListNorm);
        ListIdx:=Filtered([1..Length(ListNorm)], x->ListNorm[x] = MinNorm);
        ListPointSel:=EXTred{ListIdx};
        ListPointSelB:=Filtered(ListPointSel, x->Position(EXTred, 2*ePointRed - x) <> fail);
        ePt1:=ListPointSelB[1];
        for iPt in [1..Length(ListPointSelB)]
        do
            ePt2:=ListPointSelB[iPt];
            eEqua:=[];
            for eMat in TheBasis
            do
                Add(eEqua, (ePt1 - ePointRed) * eMat * (ePt1 - ePointRed)  -  (ePt2 - ePointRed) * eMat * (ePt2 - ePointRed));
            od;
            Add(ListEqua, eEqua);
        od;
    od;
