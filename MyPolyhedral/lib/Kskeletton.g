...
GetCompleteListFacesCpp:=function(FAC, EXT, GRPfac, method_spann, method_final)
    local FileEXT, FileFAC, FileGRP, FileNML, FileOUT, FileERR, output, TheCommand, TheOutput;
    #
    FileEXT:=Filename(POLYHEDRAL_tmpdir, "FileEXT");
    SYMPOL_PrintMatrix(FileEXT, EXT);
    #
    FileFAC:=Filename(POLYHEDRAL_tmpdir, "FileFAC");
    SYMPOL_PrintMatrix(FileFAC, FAC);
    #
    FileGRP:=Filename(POLYHEDRAL_tmpdir, "FileGRP");
    SYMPOL_PrintGroup(FileGRP, Length(FAC), GRPfac);
    #
    FileOUT:=Filename(POLYHEDRAL_tmpdir, "FileOUT");
    FileERR:=Filename(POLYHEDRAL_tmpdir, "FileERR");
    #
    FileNML:=Filename(POLYHEDRAL_tmpdir, "FileNML");
    output:=OutputTextFile(FileNML, true);
    AppendTo(output, "&PROC\n");
    AppendTo(output, " FACfile = \"", FileFAC, "\"\n");
    ...
    AppendTo(output, " method_spann = \"", method_spann, "\"\n");
    AppendTo(output, " method_final = \"", method_final, "\"\n");
    AppendTo(output, "/\n");
    CloseStream(output);
    #
#    TheCommand:=Concatenation(FileFaceLatticeGen, " ", FileNML, " 2> ", FileERR);
    TheCommand:=Concatenation(FileFaceLatticeGen, " ", FileNML);
    Print("TheCommand=", TheCommand, "\n");
    Exec(TheCommand);
    #
    TheOutput:=ReadAsFunction(FileOUT)();
...
SPAN_face_LinearProgramming:=function(face, Stabface, ListFacets, GroupFac, BoundingSet)
  ...
  do
    if Treated[iFac]=0 then
      ...
      od;
      #Print("ListVectSpann=\n");
      #PrintArray(ListVectSpann);
      BasisSpann:=RowReduction(ListVectSpann).EXT;
      #Print("BasisSpann=\n");
      #PrintArray(BasisSpann);
      ...
      od;
      #Print("ListVectors=\n");
      #PrintArray(ListVectors);
      SetListVectors:=Set(ListVectors);
      #Print("SetListVectors=\n");
      #PrintArray(SetListVectors);
...
SPAN_K_Faces:=function(GroupFac, ListFace, FAC, SPAN_function)
  local TheFormal, TheSpann, RPL, iFace, eFAC, iOrb, TheRecord, ListSizes, ListRepresentent, nbFace, TheStab, ListStabilizer, eInc, OrbSize;
  ...
  do
    eInc:=RPL.FuncRecord(iOrb);
    TheStab:=Stabilizer(GroupFac, eInc, OnSets);
    OrbSize:=Order(GroupFac) / Order(TheStab);
    Add(ListRepresentent, eInc);
    Add(ListSizes, OrbSize);
    Add(ListStabilizer, TheStab);
  od;
  nbFace:=RPL.FuncNumber();
  Print("        nbFace=", nbFace, " ListSizes=", ListSizes, "\n");
...
IncompleteSkeletonSearch:=function(GroupFac, FAC, BoundingSet, LevSearch)
  local SPAN_function;
#  SPAN_function:=function(face, TheStab, FAC)
#    return SPAN_face_Redcheck(face, TheStab, FAC);
#  end;
  SPAN_function:=function(face, TheStab, FAC)
