...
ArchiveURL := "https://github.com/MathieuDutSik/Indefinite",
...
Persons := [
  rec(
    ...
    Institution := Concatenation( [
      "Institut Rudjer Boskovic Ecole, ",
...
##
AbstractHTML := "<span class=\"pkgname\">generic</span>",
...
AvailabilityTest :=
  function()
    ...
    if not ARCH_IS_UNIX() then
      Info(InfoWarning, 1, "Package `generic': non-Unix architecture");
      Info(InfoWarning, 1, "Package `generic': use at your own peril");
    fi;
    ListProg:=[];
    ListMiss:=Filtered(ListProg, file->Filename(DirectoriesPackagePrograms("generic"), file) = fail);
    for eFile in ListMiss
    do
      Info(InfoWarning, 1, "Package `generic': ", eFile, " is missing");
    od;
    if Length(ListMiss)>0 then
      Info(InfoWarning, 1, "Package `generic': binaries missing");
      Info(InfoWarning, 1, "Package `generic': some functions will not work");
    fi;
    return true;
  end,
