...
ArchiveURL := "http://mathieudutour.altervista.org/PlanGraph",
...
CommunicatedBy := "Mathieu Dutour sikiric (Institut Rudjer Bockovic)",
# format: mm/yyyy
# AcceptDate := "08/1999",
AcceptDate := "08/1066",
...
##
README_URL := "http://mathieudutour.altervista.org/PlanGraph/index.html",
PackageInfoURL := "http://mathieudutour.altervista.org/PlanGraph/PackageInfo.g",
...
##
AbstractHTML := "<span class=\"pkgname\">PlanGraph</span> is a package for \
computing with plane graphs and maps on surfaces, \
it is primarily intended for regular maps, zigzags and related formalisms \
it contains some procedure for exhaustive search on maps.",

PackageWWWHome := "http://mathieudutour.altervista.org/PlanGraph/",
...
Dependencies := rec(
  ...
  NeededOtherPackages := [ ["MyPolyhedral", ">= 0.0"], ["GRAPE", ">= 4.2"]],
  # NeededOtherPackages := [ ],
    ...
    if not ARCH_IS_UNIX() or
       ForAny( [],
                file -> Filename(DirectoriesPackagePrograms("grape"),
                                 file) = fail ) then
      Info(InfoWarning, 1, "Package `PlanGraph': non-Unix architecture or binaries not compiled");
      Info(InfoWarning, 1, "Package `PlanGraph': functions depending on exterior program will not work");
    fi;
    return true;
  end,
...
BannerString :=
"\nLoading  PlanGraph 0.8  (Planar Graph),\n\
by Mathieu.Dutour@gmail.com\n\n",

Subtitle := "Planar Graph and maps on surfaces",
...
Keywords := ["graph","geometry","polyhedra"]
