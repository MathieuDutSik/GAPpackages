...
KuplingTorusMap:=function(PL, TheMult)
  ...
  do
    ...
    do
      fVert:=PL[eVert][iAdj];
      if eVert=fVert then
        Print("Sorry, but we cannot accept this kind of map\n");
        Print(NullMat(5));
      fi;
      if fVert > eVert then
        Add(ListEdges, [eVert, fVert]);
      fi;
    od;
  od;
  ...
  TheHomologyBasisRed:=SubspaceCompletionInt(TheFaceImageRed, DimZspace);
  TheHomologyBasis:=[];
  ...
  do
    ...
    do
      if fVert>eNewVert.eVert then
        TheSign:=1;
        eEdge:=[eNewVert.eVert, fVert];
      else
        TheSign:=-1;
        eEdge:=[fVert, eNewVert.eVert];
      fi;
      posEdge:=Position(ListEdges, eEdge);
      eNewVect:=eNewVert.eVect + TheSign*ListEdgeVect[posEdge];
...
DoesItComeFromSelfDualGrid:=function(PL)
  local PLori, LPLori, test;
  PLori:=PlanGraphToPlanGraphOriented(PL);
  LPLori:=MedialToOriginalsOriented(PLori);
  test:=IsIsomorphicPlanGraphOriented(LPLori[1], LPLori[2]);
  if test=true then
    return LPLori[1];
  fi;
  return false;
